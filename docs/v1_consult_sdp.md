...
### 1.2 이 문서의 목적
위 서비스에서 **컨설턴트 챗봇 모델 개발 담당 1명이 맡는 부분만** 떼어내,
10일 안에 완주 가능한 실행 계획으로 만든다.

### 1.3 컨설턴트 챗봇 모델 개발 담당의 한 줄 정의
> **"소상공인에게 '뭘 팔아야 하는지, 어떻게 알려야 하는지'를 데이터 근거로 제안한다.
> 그리고 두 챗봇이 함께 쓰는 데이터 계층을 공급한다."**

### 1.4 ★ 이 담당자의 이중 역할 (가장 먼저 이해할 것)

> 다른 담당자와 결정적으로 다른 점이다. **이 담당자는 두 가지 일을 동시에 한다.**

```
[역할 ①] 데이터 공급자
   market_repo · review_repo · trend_repo 의 1·2층(적재+조회함수)을 소유한다.
   → 이 데이터는 컨설턴트 챗봇만 쓰는 게 아니라, 기본 챗봇도 함께 읽는다.
   → 즉 내가 늦으면 기본 챗봇 담당자가 멈춘다.

[역할 ②] 전략 제안자
   같은 데이터를 '컨설팅용'으로 해석해 마케팅 전략을 만든다.
   → 기본 챗봇은 같은 데이터를 '카피용'으로 해석한다(3층은 각자).
```

**왜 이렇게 배분됐나**: 상권·리뷰 분석이 **컨설턴트의 주력 기능**이라,
그 데이터를 가장 깊이 쓰는 사람이 적재까지 맡는 것이 자연스럽다.
...
> **본 담당자는 전민재 PM·기본 챗봇 담당자·서비스 개발 담당자가 하지 않는 기능을 개발한다.**
...
| **데이터 계층 1·2층 전체** | `market_repo`(상권 API 적재+조회) · `review_repo`(리뷰 배치 수집+감성분석+조회) · `trend_repo`(트렌드 API 적재+조회)<br>★ **두 챗봇 공용 — 기본 챗봇 담당자에게 조회 함수 제공** |
| **컨설팅용 해석 노드(3층)** | 상권·경쟁 밀도 분석 · 경쟁사 리뷰 차별점 · 시즌·수요 타이밍 |
| **★ 마케팅 전략 제안 노드** | 메뉴 · 프로모션 · 가격 · 타겟팅 — **근거 포함**(가장 중요) |
| **전략 품질 검증 노드** | 근거 유무 · 실행 가능성 · LLM Judge |
| **★ 광고 연계 노드** | **[이 전략으로 광고 만들기]** → 기본 챗봇 호출 · `source_gen_id` 기록 |
| **질문 분석 · 추천 질문 칩** | 대명사 해소 · 재작성 · 추천 질문 3종 처리 |
| **검증 함수 노드 연결** | PM이 제공하는 검증 함수 3종(보안1·보안2·self_check)을 **노드로 감싸 연결** |
...
| **규제 준수 검증 노드** | 기본 챗봇 담당자 (함수는 PM 제공) |
...
| **골든 데이터셋 기준 설계**(전체 팀 기준) | 전민재 PM 주도 ※ 컨설팅 문항은 내가 작성 |
...
> ⚠️ **경계에서 헷갈리기 쉬운 것 6가지**
> 1. **데이터 계층은 공용**: `market_repo`·`review_repo`·`trend_repo`는 **내가 만들지만 기본 챗봇도 읽는다.**
>    → 내 편의만 생각해 설계하면 안 되고, **D1에 조회 함수 시그니처를 합의**해야 한다(§2.4)
> 2. **3층 해석은 각자**: 같은 "반경 300m에 카페 12곳" 데이터라도
>    · **컨설턴트(나)** 는 *"밀도 상위 15%입니다. 선착순 할인을 제안드립니다"* 로 쓰고
>    · **기본 챗봇**은 *"경쟁 밀집 지역이니 차별점을 강조하는 카피"* 로 쓴다
>    → **원본은 공유하고, 해석 함수는 각자 만든다**
> 3. **벤치마크는 반대 구조**: **데이터 작성은 내가(또는 PM)**, **인덱싱·검색 코드는 기본 챗봇 담당자**다
> 4. **검증 3종**: **판정 함수는 PM이 만들고**, **그것을 그래프 노드로 감싸 연결하는 건 나**다
> 5. **한 파일에는 주인이 한 명**: `build_consultant.py`·`cache/`는 **내가 소유**한다.
>    `build_basic.py`는 기본 챗봇 담당자 것이므로 건드리지 않는다 → 머지 충돌 방지
> 6. **광고 연계는 "호출"까지만**: [이 전략으로 광고 만들기]를 누르면 **기본 챗봇을 호출**하는 것까지가 내 일이고,
...
| 등급 | 항목 | 판단 기준 |
| --- | --- | --- |
| 🔴 **MUST** | **`market_repo` 적재+조회**, LangGraph 골격, 질문분석, 상권·경쟁 밀도 분석, **마케팅 전략 제안**, PM 검증 함수 3종 노드 연결, API 계약 준수 | **없으면 컨설턴트 챗봇이 동작 안 함**<br>※ market_repo는 **기본 챗봇도 대기 중**이라 최우선 |
| 🟠 **SHOULD** | `trend_repo` 적재+조회, 시즌·수요 타이밍, 전략 품질 검증, **광고 연계 노드**, 추천 질문 칩, 벤치마크 KB 데이터, 골든 데이터셋·평가 | 있으면 완성도·평가점수 급상승 |
| 🟢 **COULD** | `review_repo`(리뷰 수집+감성분석), 경쟁사 리뷰 차별점 노드, RAGAS 전체 지표 | **EX-01은 [확장] 등급 — 못 해도 감점 없음** |

> **`review_cache`가 COULD인 이유**: 요구사항 명세 v10에서 **EX-01(경쟁사 리뷰 분석)은 [확장] 등급**으로 확정됐다.
> 네이버 플레이스 크롤링은 **약관·안정성 리스크**가 크고, 선택자가 자주 바뀌어 시연 당일 실패 확률이 높다.
> → **사전 배치 수집한 캐시만 사용**하고, 시간이 없으면 과감히 버린다.

### 2.4 ★ 기본 챗봇 담당자에게 제공할 조회 함수 (D1 최우선)

> **이것이 이 SDP에서 가장 먼저 해야 할 일이다.**
> 기본 챗봇 담당자의 SDP를 보면 **D8·D9에 내 함수를 기다린다.** 늦으면 남이 멈춘다.

```python
# cache/market_repo.py — 내가 제공
...
> 나는 이후 **내용만 채우면** 상대 코드는 손댈 필요가 없다.

```python
# D1에 이 상태로 먼저 커밋
def get_market(lat, lng, radius=500) -> dict:
    """상권 조회 — D3에 실제 API 연동 예정"""
...
| 테이블/데이터 | 적재 담당 | 배분 근거 |
| --- | --- | --- |
| **`market_cache`** | **나 (컨설턴트)** | **상권 분석이 컨설턴트의 주력 기능** |
| **`review_cache`** | **나 (컨설턴트)** | **EX-01이 컨설턴트 소속으로 확정**(v9 반영) |
| **트렌드 API 호출** | **나 (컨설턴트)** | 상권 API 클라이언트와 **작업 패턴이 동일**(호출·파싱·캐시) |
| 업종 벤치마크 KB **데이터** | **전민재 PM 또는 나** | **코딩이 아니라 자료 수집·정리** 작업 |
| 업종 벤치마크 **인덱싱·검색 코드** | 기본 챗봇 담당자 | FAISS 인덱싱은 **개발 작업** |
| `banned_keywords` | **전민재 PM** | 검증 함수 4종을 PM이 직접 개발하므로 사전도 PM이 설계 |
...
### 2.6 컨텍스트 노드 3층 구조 — 내 소유 범위

```
[1층] 데이터 확보  — API 호출 / 배치 수집 → DB 적재      ← 내 소유
[2층] 조회 함수    — DB에서 원본 꺼내오기                 ← 내 소유 (기본 챗봇도 사용)
[3층] 해석 노드    — 꺼낸 원본을 "컨설팅용"으로 가공       ← 내 소유
                    (기본 챗봇은 같은 원본을 "카피용"으로 별도 가공)
```

| 컨텍스트 | 1층 적재 | 2층 조회 함수 | 3층 해석 노드 |
| --- | --- | --- | --- |
| **상권·타겟** | **나** | **나** | **각자** (컨설팅용=나 / 카피용=기본 챗봇) |
| **경쟁사 리뷰** | **나** | **나** | **나만** — 기본 챗봇 미사용 |
| **시즌·이슈** | **나** | **나** | **각자** (컨설팅용=나 / 카피용=기본 챗봇) |
| 업종 벤치마크 | PM/나(데이터) | 기본 챗봇 | 기본 챗봇 |
...
### 3.2 ★ 핵심 전략 — "데이터 먼저" (이 계획의 생명줄)

> **문제**: 내가 만드는 데이터 계층을 **기본 챗봇 담당자가 기다린다.**
> 내가 전략부터 만들면, 상대는 D8·D9에 할 일이 없어진다.
...
**왜 이 순서인가**: 다른 담당자의 SDP에서 **내 함수를 D8·D9에 쓰겠다고 명시**했다.
내가 늦으면 **팀 전체 일정이 밀린다.** 내 편의보다 **팀 의존성**이 우선이다.

### 3.3 ★ 리뷰 수집은 "안전 우선"으로

> `review_cache`는 네이버 플레이스 크롤링이 필요한데, **가장 깨지기 쉬운 부분**이다.

| 원칙 | 내용 |
| --- | --- |
| **실시간 금지** | 파이프라인에서 실시간 크롤링 **절대 금지**. 사전 배치 수집만 |
| **캐시 조회만** | 챗봇은 `review_repo.get_review()` 로 **저장된 데이터만** 읽는다 |
| **실패 허용** | 크롤링이 안 되면 **리뷰 없이도 전략이 나오도록** 설계 |
| **COULD 등급** | 시간이 없으면 **과감히 버린다** (EX-01은 [확장]) |
...
```
[내가 제공]  get_market() / get_trend()          → D1에 mock 반환
[PM이 제공]  check_input / check_output / self_check → D1에 pass 반환
   ↓
...
| 일자 | 작업 내용 | 완료 기준(DoD) |
| --- | --- | --- |
| **D1** 8/10(월) | · **★ 기본 챗봇 담당자에게 `get_market()`·`get_trend()` mock 스텁 제공**(§2.4) ← **최우선**<br>· **서비스 담당과 API 계약(JSON 형식) 합의**<br>· **캐시 테이블 스키마 협의**(서비스 담당이 테이블 생성)<br>· **PM과 검증 함수 3종 인터페이스 합의 + pass 스텁 수령**<br>· **PM 주재 GraphState 공통 규격 확정**<br>· LangGraph 프로젝트 구조 + GraphState 정의<br>· 상권 API 키 발급 확인 | 그래프가 빈 노드로 START→END 실행됨<br>**상대가 내 함수를 import 가능** |
| **D2** 8/11(화) | · **★ `market_repo` 실제 구현**(상권 API 호출·파싱·적재)<br>· `get_market()` 실제 데이터 반환<br>· 좌표 반올림·TTL 캐시 정책 | "강남역 좌표" 입력 → 반경 내 매장 수 반환 |
| **D3** 8/12(수) | · **상권·경쟁 밀도 분석 노드**(3층, 컨설팅용)<br>· 밀도 순위·최근접 거리 계산<br>· 질문 분석 노드(재작성·정보충분성)<br>· PM 제공 `check_input()` 을 보안1 노드로 연결 | "주변 경쟁사는 얼마나 되나요?" → 밀도 분석 결과 |
| **D4** 8/13(목) | · **★ 마케팅 전략 제안 노드**(가장 중요)<br>· 프롬프트 설계 — 메뉴·프로모션·가격·타겟팅<br>· **근거(sources) 포함** 출력 | "우리 카페 어떻게 홍보하면 좋을까요?" → 근거 있는 전략 |
| **D5** 8/14(금) | · **`trend_repo` 구현**(트렌드 API 적재+조회)<br>· `get_trend()` 실제 데이터 반환 → **기본 챗봇 담당자에게 통보**<br>· PM 제공 `check_output()`·`self_check()` 노드 연결<br>· **API 계약 형식으로 JSON 반환**<br>· **스프린트 1 회고(KPT)** | **데이터 계층 완성 + 전략 파이프라인 관통** |
...
| **D7** 8/18(화) | · **서비스 개발 담당과 실연동**(Mock → 실제 교체)<br>· 응답 형식 불일치 대응<br>· 429 대응(요청 간격·재시도·백오프) | 서비스 화면에서 컨설턴트 답변이 표시됨 |
| **D8** 8/19(수) | · **★ 광고 연계 노드**([이 전략으로 광고 만들기])<br>· 기본 챗봇 호출 + `source_gen_id` 기록<br>· 기본 챗봇 담당자와 **연계 인터페이스 합의·테스트** | 전략 → 광고 생성으로 넘어가는 흐름 동작 |
| **D9** 8/20(목) | · **전략 품질 검증 노드**(근거 유무·실행 가능성·LLM Judge)<br>· **골든 데이터셋 컨설팅 문항 작성**(20~30문항)<br>· 평가 스크립트 작성·1차 측정 | 평가 점수 산출 |
...
| 마일스톤 | 날짜 | 판정 기준 |
| --- | --- | --- |
| **M1** 스텁 제공 + 계약·규격 확정 | 8/10 | **기본 챗봇 담당자가 내 함수를 import 가능** |
| **M2** `market_repo` 완성 | 8/11 | 실제 상권 데이터가 조회됨 |
...
| 역할 | 인원 | 나와의 관계 |
| --- | --- | --- |
| **컨설턴트 챗봇 모델 개발 담당** | **1명 (초급 AI 엔지니어)** | **본 SDP의 대상 = 나** |
| 기본 챗봇(게시물 생성) 담당 | 별도 | **내 조회 함수를 사용** · 광고 연계 상대 |
| 서비스 개발 담당 | 별도 | 내 챗봇을 **호출**함 (API 계약) · DB 테이블 생성 |
| 모델 서빙 담당 | 별도 | SDXL Triton 서빙 (내 범위 아님) |
| 인프라 담당 | 윤승준 | 배포·CI/CD |
...
| 상권 데이터 | **소상공인시장진흥공단 상권정보 OpenAPI** | 공공데이터, 무료, 안정적 |
...
| DB | **SQLAlchemy + SQLite3** | 서비스 담당이 테이블 생성, 나는 데이터 적재 |
...
- **파일 소유권**: `cache/`·`graph/build_consultant.py`·`nodes/*_for_consult.py` 는 **내가 소유**,
...
| API | 한도 | 대응 |
| --- | --- | --- |
| 상권정보 OpenAPI | **초당 30 TPS**, 반경 ≤2000m, 페이지당 ≤1000 | 배치 적재 시 sleep, 좌표 반올림 캐싱 |
| NAVER 검색어 트렌드 | **월 5만건 · API Key당 50 RPS**, 초과 시 429 | 캐시 우선 조회, 같은 키워드 재호출 금지 |
...
[서비스 개발 담당] 사용자 질문 · 사용량/크레딧 확인
        ↓
① 보안1: 입력 검증                         ※PM(전민재) 직접 개발
        ↓
  질문 분석 (재작성·정보충분성)  →(칩 클릭)→ 추천 질문 칩 처리
        ↓
② 데이터 계층 (1·2층 = 내 소유, 두 챗봇 공용)
   · market_repo   상권 API 적재 + get_market() 조회
   · review_repo   리뷰 배치 수집 + 감성분석 + 조회   [COULD]
   · trend_repo    트렌드 API 적재 + get_trend() 조회
   · (벤치마크 인덱싱은 기본 챗봇 담당 — KB 데이터만 내가 작성)
        ↓
③ 컨설팅용 해석 (3층 = 내 소유)
...
⑥ 보안2: 출력 검증                        ※PM(전민재) 직접 개발
        ↓
   self_check (근거·전략 완결성 판정)      ※PM(전민재) 직접 개발
        ↓
...
· 컨텍스트 캐싱: 같은 좌표는 market_cache 재사용 → 외부 API 호출 절감
...
│
├── cache/                            # ★★ 내 소유 (1·2층) — 두 챗봇 공용
│   ├── market_repo.py                # 상권 API 적재 + get_market() 조회   [MUST]
│   ├── trend_repo.py                 # 트렌드 API 적재 + get_trend() 조회  [SHOULD]
│   └── review_repo.py                # 리뷰 배치 수집 + 조회               [COULD]
│                                     #   ★ D1에 mock 반환 스텁으로 먼저 커밋
│
├── shared_data/                      # PM/내가 채우는 데이터
│   ├── benchmark_kb.json             # 업종 벤치마크 KB (PM 또는 내가 작성)
│   └── golden_dataset.json           # 골든 데이터셋 (PM 주도 · 컨설팅 문항은 내가)
│
└── model_consult/                    # ★ 내 작업 영역 (컨설턴트 챗봇)
    ...
    │   ├── load_market_cache.py      # 상권 데이터 적재 배치
    │   ├── load_trend_cache.py       # 트렌드 데이터 적재 배치
    │   └── load_review_cache.py      # 리뷰 사전 수집 배치          [COULD]
    │
    ├── eval/                         # ★ 평가
    │   ├── eval_strategy.py          # 전략 품질 LLM Judge
    │   ├── eval_retrieval.py         # 데이터 조회 정확도
    │   └── reports/                  # 평가 결과 · 튜닝 전후 비교표
    │
    ├── tests/
    │   ├── test_market_repo.py       # 상권 조회 함수 테스트
...
**구축 계획 (D9, 초급자 현실 기준)**
> ※ **전체 기준 설계는 전민재 PM 주도**. 나는 **컨설팅 영역 문항 20~30개**를 작성한다.
...
2. "우리 카페 어떻게 홍보하면 좋을까요?" → 근거 포함 전략 2~3개
3. "어떤 이벤트가 효과적일까요?" → 시즌 데이터 반영 전략
4. 데이터 없는 지역 질문 → "정보가 부족합니다" 안내 (지어내지 않음)
5. 불법 키워드 포함 요청 → 보안1에서 차단 (PM 함수 연동 확인)
6. [이 전략으로 광고 만들기] 클릭 → 기본 챗봇으로 전달 확인
7. source_gen_id가 generations에 기록되는지 확인
8. 기본 챗봇 담당자가 내 get_market()·get_trend() 를 정상 사용하는지 확인
9. 서비스 담당 화면에서 위 결과가 그대로 표시되는지 확인
```
...
| R4 | **OpenAI $30 한도 초과 → 키 비활성화** | 중 | **치명적** | 직렬 호출·sleep·재생성 상한. 대량 평가 전 PM 사전 공유 |
| R5 | **트렌드 API 한도 초과**(월 5만·50 RPS) | 중 | 중 | **캐시 우선 조회**. 같은 키워드 재호출 금지 |
...
| R9 | **PM 검증 함수 제공 지연** | 중 | 중 | **스텁 선제공 원칙**(§3.6) — pass 스텁으로 노드 미리 연결 |
| R10 | **초급자 LangGraph 난이도** | 중 | 중 | 파트3 학습자료 재활용. **기본 챗봇 담당자와 골격 공유**. 30분 룰 |
| R11 | **일정 지연** | 중 | 높음 | COULD부터 버린다(§2.3). 8/22 버퍼 확보 |

> **가장 큰 리스크는 R1이다.**
> 다른 담당자와 달리, **내가 늦으면 남이 멈춘다.** 그래서 §3.2 "데이터 먼저" 전략이 이 SDP의 핵심이다.
...
| **서비스 개발 담당** | API 계약(JSON) · **캐시 테이블 스키마 요청**(내가 채울 컬럼) | **D1 필수** |
| **PM(전민재)** | **검증 함수 3종 인터페이스 + pass 스텁 수령** · GraphState 규격 · 골든 데이터셋 기준 · OpenAI 사용량 | **D1 필수** / 매일 |
| **인프라 담당(윤승준)** | 배치 스크립트 실행 환경(cron 등) 문의 | D2 이후 |
...
| 3 | **캐시 테이블 스키마**(서비스 담당이 생성) | 협의 필요 | **8/10 D1** |
| 4 | **검증 함수 3종 인터페이스**(PM) | PM과 합의 필요 | **8/10 D1** |
| 5 | **GraphState 공통 규격**(두 챗봇 공유 필드) | PM 주재 확정 필요 | **8/10 D1** |
| 6 | **상권 API 인증키 발급** | 공공데이터포털 신청 | **8/10 D1** |
| 7 | **NAVER API HUB 트렌드 키 발급** | NCP 계정 필요 | **8/10 D1** |
| 8 | **광고 연계 인터페이스**(§6.3) | 기본 챗봇 담당자와 합의 | 8/19 D8 |
| 9 | 컨설턴트 챗봇 과금 여부 | 미정 (팀 결정) | 8/10 D1 |
| 10 | `review_cache` 진행 여부 | 본 SDP는 **[COULD]** 제안 | 8/20 D9 |
| 11 | 골든 데이터셋 컨설팅 문항 수(30) | 본 SDP 제안 | 8/20 D9 |
...
- [ ] **★ 기본 챗봇 담당자가 내 `get_market()`·`get_trend()` 를 정상 사용한다**
...
- **서비스 개발 담당 SDP** (`sd_sdp.md`) — API 계약 상대
