...
### 1.5 ★ (v1.1 신규) 데이터 확보 방식 한 줄 요약

> **"DB 캐시 테이블을 유지하되, Cache-Aside 방식으로 캐시에 없으면 API를 실시간 호출해 채운다.
> 확보한 데이터와 사용자 질문을 조합해 LLM이 전략을 추론한다."**
...
| **데이터 계층 1·2층 전체** | `market_repo`·`trend_repo`·`review_repo`<br>**★ Cache-Aside 패턴**(캐시 우선 조회 → 미스 시 API 실시간 호출 → 적재)(§2.7)<br>★ **두 챗봇 공용 — 기본 챗봇 담당자에게 조회 함수 제공** |
...
| **캐시 테이블 스키마 생성**(DDL) | **서비스 개발 담당자** ※ 적재·조회 로직은 내가 |
...
> 7. **★ 캐시 테이블은 "생성"과 "사용"이 다르다**(v1.1): **테이블 DDL은 서비스 담당**이 만들고,
>    **데이터 적재·조회 로직(Cache-Aside)은 내가** 구현한다
> 8. **★ 지오코딩은 내 일이 아니다**(v1.1): 회원가입 시 **서비스 담당이 1회 호출**해 `users.lat/lng`에 저장하고,
>    나는 **요청 JSON에 실려 온 좌표를 쓰기만** 한다(§2.8)
...
| 등급 | 항목 | 판단 기준 |
| --- | --- | --- |
| 🔴 **MUST** | **`market_repo` Cache-Aside 구현**, LangGraph 골격, 질문분석, 상권·경쟁 밀도 분석, **마케팅 전략 제안**, PM 검증 함수 3종 노드 연결, API 계약 준수 | **없으면 컨설턴트 챗봇이 동작 안 함**<br>※ market_repo는 **기본 챗봇도 대기 중**이라 최우선 |
| 🟠 **SHOULD** | `trend_repo` Cache-Aside, 시즌·수요 타이밍, 전략 품질 검증, **광고 연계 노드**, 추천 질문 칩, 벤치마크 KB 데이터, 골든 데이터셋·평가 | 있으면 완성도·평가점수 급상승 |
| 🟢 **COULD** | `review_repo`(리뷰 배치 수집+감성분석), 경쟁사 리뷰 차별점 노드, 채팅 중 지역 변경 지오코딩, RAGAS 전체 지표 | **EX-01은 [확장] 등급 — 못 해도 감점 없음** |
...
| **`trend_cache`** | **나 (컨설턴트)** | 상권 API 클라이언트와 **작업 패턴이 동일**(호출·파싱·캐시) |
...
| **캐시 테이블 DDL(스키마 생성)** | **서비스 개발 담당자** | DB 스키마는 서비스 담당 영역 — 나는 **적재·조회 로직만** |
...
```
[1층] 데이터 확보  — Cache-Aside(캐시 미스 시 API 실시간 호출) → DB 적재   ← 내 소유
[2층] 조회 함수    — get_market() / get_trend() / get_review()             ← 내 소유 (기본 챗봇도 사용)
...
### 2.7 ★ (v1.1 신규) Cache-Aside 패턴 — 캐시와 실시간 호출의 결합
...
def get_market(lat, lng, radius=500) -> dict:
    """상권 조회 — Cache-Aside(캐시 우선, 미스 시 API 호출 후 적재)"""
...
3. **발표 설명이 명확해진다** — *"캐시 우선 조회로 외부 API 한도를 방어하면서, 미조회 데이터는 실시간으로 채웁니다"*

> ⚠️ **리뷰는 예외**: `review_repo`는 Cache-Aside가 아니라 **사전 배치 수집 전용**이다.
> 크롤링은 수십 초가 걸려 실시간 호출이 불가능하므로, **캐시에 없으면 그냥 없는 대로 진행**한다(§3.3).

### 2.8 ★ (v1.1 신규) 지오코딩 담당 구분 — 내가 호출하지 않는다
...
| **Cache-Aside 예외** | 상권·트렌드와 달리 **캐시 미스 시 API를 부르지 않는다.** 없으면 없는 대로 진행 |
...
| 일자 | 작업 내용 | 완료 기준(DoD) |
| --- | --- | --- |
| **D1** 8/10(월) | · **★ 기본 챗봇 담당자에게 `get_market()`·`get_trend()` mock 스텁 제공**(§2.4) ← **최우선**<br>· **서비스 담당과 API 계약(JSON 형식) 합의**<br>· **★ 서비스 담당에게 확인 2건**: ①요청에 `store.lat/lng` 포함 여부(§2.8) ②캐시 테이블 DDL 생성 요청<br>· **PM과 검증 함수 3종 인터페이스 합의 + pass 스텁 수령**<br>· **PM 주재 GraphState 공통 규격 확정**<br>· LangGraph 프로젝트 구조 + GraphState 정의<br>· 상권 API·트렌드 API 키 발급 확인 | 그래프가 빈 노드로 START→END 실행됨<br>**상대가 내 함수를 import 가능** |
| **D2** 8/11(화) | · **★ `market_repo` Cache-Aside 구현**(§2.7)<br>&nbsp;&nbsp;— MarketCache ORM 모델(서비스 담당 테이블 사용)<br>&nbsp;&nbsp;— 캐시 조회 → 미스 → 상권 API 호출 → 적재<br>&nbsp;&nbsp;— 좌표 반올림(소수 3자리) 캐시 키 · **TTL 7일** | "강남역 좌표" 입력 → 반경 내 매장 수 반환<br>**2회차 호출 시 API 미호출(캐시 히트) 확인** |
...
| **D5** 8/14(금) | · **`trend_repo` Cache-Aside 구현**(TTL 7일 · 월 5만 한도 방어)<br>· `get_trend()` 실제 데이터 반환 → **기본 챗봇 담당자에게 통보**<br>· PM 제공 `check_output()`·`self_check()` 노드 연결<br>· **API 계약 형식으로 JSON 반환**<br>· **스프린트 1 회고(KPT)** | **데이터 계층 완성 + 전략 파이프라인 관통** |
...
| 서비스 개발 담당 | 별도 | 내 챗봇을 **호출**함 (API 계약) · **캐시 테이블 DDL 생성** · **지오코딩** |
...
| DB | **SQLAlchemy + SQLite3** | **테이블 DDL은 서비스 담당**, 적재·조회는 나 |
...
![파이프라인](pipeline_consult_sdp.png)
...
① 보안1: 입력 검증                         ※PM(전민재) 직접 개발
  ...
  질문 분석 (재작성·정보충분성)  →(칩 클릭)→ 추천 질문 칩 처리
        ↓
② 데이터 계층 (1·2층 = 내 소유, 두 챗봇 공용) — ★ Cache-Aside
   · market_repo   TTL 7일 · 캐시 미스 → 상권 API 실시간 호출 → 적재
   · trend_repo    TTL 7일 · 캐시 미스 → 트렌드 API 실시간 호출 → 적재
...
> **★ `lat`·`lng` 필수 확인**(§2.8): 서비스 담당이 회원가입 시 지오코딩해 저장한 값이 여기 실려 온다.
> 이 값이 없으면 내가 상권 조회를 할 수 없으므로 **D1에 반드시 확인**한다.
...
6. 불법 키워드 포함 요청 → 보안1에서 차단 (PM 함수 연동 확인)
7. [이 전략으로 광고 만들기] 클릭 → 기본 챗봇으로 전달 확인
8. source_gen_id가 generations에 기록되는지 확인
9. 기본 챗봇 담당자가 내 get_market()·get_trend() 를 정상 사용하는지 확인
...
| 6 | **검증 함수 3종 인터페이스**(PM) | PM과 합의 필요 | **8/10 D1** |
| 7 | **GraphState 공통 규격**(두 챗봇 공유 필드) | PM 주재 확정 필요 | **8/10 D1** |
