...
> **문서 버전**: SDP-Model v1.6 (검증 함수 스텁 선제공 원칙 추가)

---

## 1. 프로젝트 개요 — "우리가 왜 이 일을 하는가?"
...
### 1.2 이 문서의 목적
위 서비스에서 **기본 챗봇(게시물 생성) 모델 개발 담당 1명이 맡는 부분만** 떼어내,
10일 안에 완주 가능한 실행 계획으로 만든다.

### 1.3 기본 챗봇 모델 개발 담당의 한 줄 정의
> **"소상공인의 요청을 받아, 상권·트렌드를 반영한 광고 카피 3안과 이미지를 만들어내는 '두뇌'를 만든다.
> 화면과 과금은 만들지 않는다."**

---

## 2. 범위(Scope) — "무엇을 만들고, 무엇을 만들지 *않는가*?"
...
| **검증 함수 노드 연결** | PM이 제공하는 검증 함수 4종을 **LangGraph 노드로 감싸 연결**<br>⚠️ **함수 구현 자체는 전민재 PM 담당**(§2.4) |
| **컨텍스트 요약 노드(3층)** | **상권**·**시즌/이슈** — 타인의 조회 함수를 받아 **카피용 요약 노드만** 구현<br>**업종 벤치마크** — **인덱싱·검색·요약**(데이터 작성은 PM/컨설턴트)(§2.6) |
| **평가 스크립트 코드** | HitRate·MRR·LLM Judge·CLIP Score **측정 코드**<br>※ **골든 데이터셋 문항 작성은 PM 주도**(§2.7) |
| **OpenAI 429 대응** | 모델 호출 측 요청 간격·재시도·토큰 절감 |
| **API 계약 준수** | 서비스 개발 담당이 호출할 응답 JSON 형식 준수 |
| **★ 프롬프트 튜닝** | 카피 품질 개선 — **재배분으로 확보한 시간을 여기에 재투입**(§2.7) |

### 2.2 ❌ 미포함 범위 (내가 만들지 않는다)

| 미포함 항목 | 담당 |
| --- | --- |
| **컨설턴트 챗봇 기능 구현 전체** | **다른 모델 개발 담당자** |
| 마케팅 전략 제안 노드(메뉴·프로모션·타겟팅) | 다른 모델 개발 담당자 |
| **상권·리뷰 데이터의 '컨설팅용 해석'**(전략 제안 형태로 가공) | **다른 모델 개발 담당자** |
| **경쟁사 리뷰 노드 전체**(적재·조회·요약 3층 모두) | **다른 모델 개발 담당자** ← v1.4 이관 |
| 경쟁사 리뷰 차별화 전략 요약(EX-01) | 다른 모델 개발 담당자 |
| **`market_cache` 적재 + 조회 함수**(1·2층) | **다른 모델 개발 담당자** ← v1.4 명확화 |
| **`review_cache` 적재 + 조회 함수**(1·2층) | **다른 모델 개발 담당자** ← v1.4 명확화 |
| **트렌드 API 호출 + `get_trend()` 조회 함수**(1·2층) | **다른 모델 개발 담당자** ← **v1.5 이관**(§2.7) |
| **업종 벤치마크 KB 데이터 작성**(JSON 내용 채우기) | **PM 또는 컨설턴트** ← **v1.5 이관**(§2.7) |
| **골든 데이터셋 문항 설계·작성**(55문항) | **전민재 PM 주도** ← **v1.5 이관**(§2.7) |
| **`banned_keywords` 사전 설계·작성** | **전민재 PM** ← v1.3 이관 |
| **보안1(입력 검증) 판정 함수 구현** | **전민재 PM** ← v1.3 이관 |
| **규제 준수 검증 판정 함수 구현** | **전민재 PM** ← v1.3 이관 |
| **보안2(출력 검증) 판정 함수 구현** | **전민재 PM** ← v1.3 이관 |
| **self_check 판정 함수 구현** | **전민재 PM** ← v1.3 이관 |
...
| GCP VM 배포 · CI/CD | 인프라 담당(윤승준) |

> ⚠️ **경계에서 헷갈리기 쉬운 것 7가지**
> 1. **캐시 테이블**: **테이블을 만드는 건 서비스 담당**, **데이터를 채우는 건 담당별 분담**이다(§2.5). → D1에 스키마 협의 필수
> 2. **이미지 생성**: **호출 인터페이스는 내가**, **SDXL 서빙 최적화는 서빙 담당**이다
> 3. **검증 4종**: **판정 함수는 PM이 만들고**, **그것을 그래프 노드로 감싸 연결하는 건 나**다(§2.4)
...
>    → **원본은 공유하고, 요약 함수는 각자 만든다.** 그래야 "형식 바꿔달라"는 요청으로 서로 멈추지 않는다
> 5. **한 파일에는 주인이 한 명**: `build_basic.py`·`state.py`는 **내가 소유**한다. PM은 함수만 제공하고 그래프는 건드리지 않는다 → 머지 충돌 방지
> 6. **컨텍스트 노드의 3층 구조**(§2.6): **1층 적재 + 2층 조회 함수는 같은 사람이 소유**하고, **3층 요약 노드만 각자** 만든다.
>    → 데이터를 **넣은 사람이 꺼내는 문까지** 만들어야, 저장 형식이 바뀔 때 한 사람만 고치면 된다
> 7. **평가는 "코드"와 "데이터"를 분리**(§2.7): **측정 스크립트 코드는 내가**, **골든 데이터셋 문항은 PM**이 만든다.
>    → **만든 사람이 자기 채점 기준을 만들면 편향**이 생기므로, 검증 기준은 제3자가 정하는 게 원칙이다

### 2.3 우선순위 — MUST / SHOULD / COULD

> 10일 안에 다 못 만들 수 있다. **미리 버릴 순서를 정해둔다.**

| 등급 | 항목 | 판단 기준 |
| --- | --- | --- |
| 🔴 **MUST** | LangGraph 골격, 질문분석, 채널확정, **카피 생성(3안)**, 랭킹, 채널리포맷, **PM 검증 함수 4종 노드 연결**, API 계약 준수 | **없으면 챗봇이 동작 안 함** |
| 🟠 **SHOULD** | 이미지 생성 연동, 이미지 자동 검수, 벤치마크 인덱싱·검색, 시즌·이슈 요약 노드, **상권 카피용 요약 노드**, 평가 스크립트, **★프롬프트 튜닝** | 있으면 완성도·평가점수 급상승 |
| 🟢 **COULD** | SDXL 자체 호스팅 교체, RAGAS 전체 지표, 재생성 루프 고도화 | **못 해도 감점 없음** |

### 2.4 ⚠️ (v1.3 개정) 검증 4종 — 전민재 PM 직접 개발

> **대상**: 보안1(입력 검증) · 규제 준수 검증 · 보안2(출력 검증) · **self_check(모델 답변 검증)**
> **개발 주체**: **전민재 PM이 판정 함수를 직접 구현**한다.
> **나의 역할**: 제공받은 함수를 **LangGraph 노드로 감싸 파이프라인에 연결**한다.

**왜 PM이 직접 개발하는가 (배분 근거)**
1. **정책 판단 영역** — "무엇을 차단할 것인가"는 코딩보다 **서비스 기획 판단**이다. 코드잇 가이드의 PM 역할에도 *"서비스 기획"*, *"서비스 테스트 및 평가"*가 명시돼 있다.
2. **의존성이 낮다** — 입력은 문자열, 출력은 판정 결과. 로직이 독립적이라 **PM의 조율 업무와 병행 가능**하다.
3. **기본 챗봇 담당의 부하 완화** — 덜어낸 시간을 **카피 프롬프트 튜닝(D10)** 에 쓸 수 있다.
...
> **반환 형식을 통일한 이유**: 네 함수를 **같은 방식으로 노드에 감쌀 수 있어** 연동 코드가 단순해진다.

**★★ (v1.6 신규) 스텁 선제공 원칙 — D1에 4개 함수를 "껍데기"로 먼저 만들어 둔다**

> **문제**: 함수마다 완성 시점이 다르다(`check_input`은 D2, `check_output`·`self_check`는 D5).
> 완성된 것만 제공하면, 담당자는 **나머지를 import조차 못 해 기다리게 된다.**
>
> **해법**: **D1에 PM이 4개 함수를 전부 `pass` 반환 스텁으로 만들어 커밋**한다.
> 담당자는 D1부터 **네 함수를 모두 import해 노드를 연결**할 수 있고,
> PM은 이후 **함수 내용만 채워 넣으면** 담당자 코드는 손대지 않아도 된다.

```python
# validation/security.py — D1에 이 상태로 먼저 커밋 (PM)
...
> → PM이 미리 만들어두고, 나는 **D5에 연결**한다. 그전까지는 **통과(pass-through) 처리**해 서로 막지 않는다.

### 2.5 데이터 적재 재배분 (v1.2 → v1.5 갱신)

> **"그 데이터를 가장 깊이 쓰는 사람이 채운다"**는 원칙에 따라 배분한다.

| 테이블/데이터 | 적재 담당 | 배분 근거 |
| --- | --- | --- |
| 업종 벤치마크 KB **데이터** | **PM 또는 컨설턴트** ← v1.5 이관 | **코딩이 아니라 자료 수집·정리** 작업 |
| 업종 벤치마크 **인덱싱·검색 코드** | **나 (기본 챗봇)** | FAISS 인덱싱은 **개발 작업** |
| `banned_keywords` | **전민재 PM** ← v1.3 이관 | 이를 사용하는 **검증 함수 4종을 PM이 직접 개발**하므로 사전도 PM이 설계 |
| `market_cache` | **컨설턴트 챗봇 담당** | **상권 분석이 컨설턴트의 주력 기능**(추천 질문 "주변 경쟁사는 얼마나 되나요?") |
| `review_cache` | **컨설턴트 챗봇 담당** | **EX-01(경쟁사 리뷰 분석)이 컨설턴트 소속으로 확정**됨(v9 반영) |
| **트렌드 API 호출** | **컨설턴트 챗봇 담당** ← v1.5 이관 | 상권 API 클라이언트와 **작업 패턴이 동일**(호출·파싱·캐시). 한 사람이 묶어 만드는 게 빠름 |
...
> **이 재배분의 효과 3가지**
> 1. **업무량 균형** — 내가 카피·이미지·평가에 집중할 시간이 확보된다
> 2. **컨설턴트 담당의 자립** — 내 적재를 기다리지 않고 본인 일정으로 진행 가능
> 3. **데이터 품질 향상** — 자기가 쓸 데이터를 자기가 채우면 필드 부족 시 즉시 보완

### 2.6 ★ (v1.4 신규) 컨텍스트 노드 3층 구조 — 업무 분배
...
[3층] 요약 노드    — 꺼낸 원본을 "내 챗봇이 쓸 형태"로 가공  (각자)
```

**★ 분배 원칙 한 줄**
> **1층과 2층은 같은 사람이 소유한다. 3층 요약 노드만 각자 만든다.**

**분배표 (v1.5 갱신)**

| 컨텍스트 | 1층 적재 | 2층 조회 함수 | 3층 요약 노드 |
| --- | --- | --- | --- |
| **상권·타겟** | 컨설턴트 | **컨설턴트** | **각자** (기본용=나 / 컨설팅용=컨설턴트) |
| **경쟁사 리뷰** | 컨설턴트 | **컨설턴트** | **컨설턴트만** — 기본 챗봇 미사용 |
| **업종 벤치마크** | **PM/컨설턴트**(데이터) | **나**(인덱싱·검색) | **나** |
| **시즌·이슈** | **컨설턴트** ← v1.5 | **컨설턴트** ← v1.5 | **나** |
...
3. **경쟁사 리뷰는 기본 챗봇 몫이 아예 없다** — EX-01은 **컨설턴트 소속·[확장] 등급**으로 확정됐으므로, 나는 **3층 요약 노드조차 만들지 않는다.**
...
[컨설턴트 소유]
  ...
  cache/trend_repo.py          ← 트렌드 API 호출 + 조회 함수 (1·2층) ← v1.5
...
[내 소유]
  ...
  nodes/trend_for_copy.py      ← 카피용 요약 (3층), trend_repo를 import만 ← v1.5
  ※ review 관련 파일은 만들지 않음
```

**★ D1에 받을 조회 함수 시그니처 (컨설턴트 담당 제공)**
```python
# cache/market_repo.py — 컨설턴트 담당 제공
...
# cache/trend_repo.py — 컨설턴트 담당 제공 (v1.5 신규)
...
> **이 반환 형식만 정해지면**, 컨설턴트의 적재가 끝나기 전에도 **mock으로 요약 노드를 먼저 만들 수 있다.**
> 서로 기다리지 않는 구조가 여기서도 적용된다.
> ※ **컨설턴트 담당에게도 §2.4의 스텁 선제공 원칙을 동일하게 요청**한다(D1에 `pass`/mock 반환으로 먼저 커밋).

**이 분배로 줄어든 내 작업 (v1.4 → v1.5 누적)**
- `context_review.py` **삭제** — 경쟁사 리뷰는 기본 챗봇 미사용 (v1.4)
- 상권 **조회 함수 불필요** — 컨설턴트가 제공, 나는 요약만 (v1.4)
- **트렌드 API 클라이언트 불필요** — 컨설턴트가 제공, 나는 요약만 (v1.5)
- **벤치마크 KB 데이터 작성 불필요** — PM/컨설턴트가 작성, 나는 인덱싱·검색만 (v1.5)

### 2.7 ★ (v1.5 신규) 업무 재배분 3건 — 확보 시간의 재투입
...
| # | 넘기는 작업 | 받는 사람 | 근거 |
| --- | --- | --- | --- |
| 1 | **트렌드 API 호출 + 조회 함수**(1·2층) | **컨설턴트 담당** | 상권 API 클라이언트와 **작업 패턴이 동일**(호출·파싱·캐시·재시도). 한 사람이 두 개를 만드는 게 두 사람이 하나씩 만드는 것보다 빠르다 |
| 2 | **골든 데이터셋 문항 설계·작성**(55문항) | **전민재 PM 주도** | ① *"무엇이 좋은 답인가"* 를 정하는 **검증 기준 설계**로, 코드잇 가이드의 PM 역할 **"서비스 테스트 및 평가"** 에 해당<br>② **만든 사람이 자기 채점 기준을 만들면 편향**이 생긴다. 잘 만든 것 위주로 문항을 짜게 됨<br>③ **거부 10문항은 PM의 검증 함수를 검증**하는 용도라 이미 PM 영역 |
| 3 | **업종 벤치마크 KB 데이터 작성**(JSON 내용) | **PM 또는 컨설턴트** | *"업종별 잘 쓴 광고 카피 예시"* 수집·정리는 **코딩이 아니라 자료 작업**이다. 반면 **FAISS 인덱싱 코드는 개발**이므로 내가 유지 |

**★ 절대 넘기지 않는 것 3가지 (반대 근거)**

| 유지 항목 | 왜 넘기면 안 되는가 |
| --- | --- |
| **카피 생성 노드 + 프롬프트 튜닝** | **기본 챗봇의 존재 이유**이자 D10 튜닝과 한 몸. 만든 사람이 튜닝해야 *"이 표현을 바꾸니 결과가 이렇게 달라진다"* 는 감이 생긴다. 남이 만들면 튜닝 때마다 왕복 발생 |
| **LangGraph 그래프 조립** | `build_basic.py`는 노드를 만들 때마다 고치는 파일. **소유자가 한 명**이어야 머지 충돌이 없다 |
...
> ⚠️ **PM 업무 과부하 주의**
> PM은 이미 **검증 함수 4종**을 맡았다. 여기에 **골든 데이터셋 55문항 + 벤치마크 KB 데이터**까지 얹으면 부담이 크다.
> → **권고**: 골든 데이터셋은 PM이 **기준(유형별 문항 수·정답 판정 기준)만 정하고**,
>   실제 55문항 작성은 **팀원이 나눠서**(각 15문항) 쓰고 **PM이 검수**하는 방식을 권한다.
>   그래야 PM이 조율 업무를 놓지 않는다.

---

## 3. 개발 방법론 — "어떤 과정을 거쳐 만들 것인가?"

### 3.1 애자일 · 2 스프린트 구성
- 프로젝트 팁의 **타임박싱(Time-boxing)** 원칙에 따라, 기간을 넘기면 기능을 버리고 다음으로 넘어간다.
- **스프린트 1**(8/10~8/14): **텍스트(카피) 파이프라인 관통**
- **스프린트 2**(8/17~8/21): 이미지 붙이기 + 평가 + 서비스 연동 + **카피 튜닝**
- **8/22(토)**: 통합 테스트 + 버퍼

### 3.2 ★ 핵심 전략 — 단계적 완성 (이 계획의 생명줄)

> **문제**: 카피 생성 + 이미지 생성 + 평가를 동시에 하면 초급자는 셋 다 미완성으로 끝난다.
> **해법**: 1주차엔 **이미지를 아예 건드리지 않는다.**
...
| **2차(도전)** | **SDXL 자체 호스팅** | D10 이후 / COULD | 모델 서빙 담당이 Triton에 올리면 **엔드포인트만 교체** |
...
최소 2일 단위로 GitHub에 머지한다. 특히 **서비스 개발 담당과의 연동부**는 빨리 합칠수록 좋다.

### 3.6 ★ (v1.6 신규) 스텁 우선(Stub-First) 협업 원칙
...
[해법] D1에 스텁부터 제공
   PM      → check_input / check_regulation / check_output / self_check  (모두 pass 반환)
...
> §3.2의 단계적 완성 전략과 함께, 이 SDP의 **2대 협업 안전장치**다.

---

## 4. 일정 및 마일스톤 — "언제까지 무엇을 해내야 하는가?"

> **실작업일 계산**: 8/10~8/22 중 주말 제외 → **실작업 10일 + 8/22(토) 버퍼**
> ※ 2026년 8월 15일(광복절)은 **토요일**이라 평일 손실 없음
> ★ **v1.5 재배분으로 약 2일 확보 → D3 확장 + D10 튜닝에 재투입**(§2.7)

### 4.1 스프린트 1 — 텍스트 파이프라인 관통 (8/10 월 ~ 8/14 금)

| 일자 | 작업 내용 | 완료 기준(DoD) |
| --- | --- | --- |
| **D1** 8/10(월) | · **서비스 담당과 API 계약(JSON 형식) 합의** ← 최우선<br>· **캐시 테이블 스키마 협의**(내가 읽을 컬럼 확인)<br>· **컨설턴트 담당과 3층 분배 확정 + `get_market()`·`get_trend()` 시그니처 수령**(§2.6)<br>· **⚠️ PM과 검증 함수 4종 인터페이스 합의**(§2.4)<br>· **★ PM·컨설턴트에게 스텁 선제공 요청**(§3.6) — D1 중 `pass`/mock 커밋<br>· **PM과 골든 데이터셋 기준 합의**(§2.7)<br>· **PM 주재 GraphState 공통 규격 확정**<br>· LangGraph 프로젝트 구조 + GraphState 정의<br>· OpenAI 키 연결 테스트 | 그래프가 빈 노드로 START→END 실행됨<br>**타인 함수 6종을 전부 import 가능** |
| **D2** 8/11(화) | · 질문 분석 노드(재작성·정보충분성)<br>· **PM 제공 `check_input()` 을 보안1 노드로 감싸 연결**<br>· **★ `check_output()`·`self_check()` 도 이 시점엔 `pass` 스텁 상태이므로,<br>&nbsp;&nbsp;노드 연결까지 미리 해둔다**(D5에 PM이 내용만 채우면 자동 반영)<br>· (PM 함수 미완 시) **통과 처리 스텁**으로 두고 진행 | 질문 분석 노드 동작 / **검증 노드 4개 자리 모두 확보** |
...
| **D4** 8/13(목) | · 배리언트 랭킹 노드(LLM Judge)<br>· **PM 제공 `check_regulation()` 을 규제검증 노드로 연결**<br>· 재생성 루프(품질미달·규제위반) | 품질 미달 카피가 재생성됨 |
| **D5** 8/14(금) | · 채널별 리포맷(인스타/블로그/현수막)<br>· **PM이 `check_output()`·`self_check()` 내용을 채움 → 내 노드는 수정 불필요**(D2에 연결 완료)<br>· 동작 확인 및 판정 결과 처리 검증<br>· **API 계약 형식으로 JSON 반환**<br>· **스프린트 1 회고(KPT)** | **텍스트 파이프라인 E2E 관통 완료** |

> **스프린트 1 목표(가장 중요)**: **"이미지는 없지만 카피는 끝까지 나오는 챗봇"**.
...
| **D7** 8/18(화) | · **서비스 개발 담당과 실연동**(Mock → 실제 교체)<br>· 응답 형식 불일치 대응<br>· 429 대응(요청 간격·재시도·백오프) | 서비스 화면에서 내 챗봇 결과가 표시됨 |
| **D8** 8/19(수) | · 이미지 자동 검수(CLIP Score·금지요소)<br>· **벤치마크 인덱싱·검색 코드**(데이터는 PM/컨설턴트 제공분 사용)<br>· **시즌·이슈 카피용 요약 노드**(`get_trend()` 연동)<br>· **★확보 시간: 컨텍스트 주입 전후 카피 비교 실험** | 벤치마크·트렌드가 카피에 반영됨<br>주입 전후 차이를 **문서로 기록** |
| **D9** 8/20(목) | · **평가 스크립트 코드 작성**(HitRate·MRR·LLM Judge)<br>· **PM 제공 골든 데이터셋으로 1차 측정**<br>· **상권 카피용 요약 노드**(`get_market()` 연동) | 평가 스크립트 1회 실행 → 점수 산출 |
...
- 팀 전체 통합 테스트(§8 시나리오)
- 버그 수정 · 발표용 시연 리허설
- **버퍼**: 앞선 일정이 밀렸다면 이 날 흡수

### 4.4 마일스톤 요약

| 마일스톤 | 날짜 | 판정 기준 |
| --- | --- | --- |
| **M1** API 계약·스키마·3층 분배·검증 인터페이스·평가 기준 확정 + **스텁 6종 수령** | 8/10 | 서비스·컨설턴트 담당·PM과 합의 완료 |
...
| **M5** 서비스 연동 완료 | 8/18 | 서비스 화면에 내 결과가 표시됨 |
| **M6** 평가 수치 확보 | 8/20 | 골든 데이터셋 기반 점수 산출 |
| **M7** 튜닝 성과 확보 | 8/21 | **튜닝 전후 점수 비교표** 완성 |

---

## 5. 자원 및 역할 — "누가, 무엇을 가지고 이 일을 하는가?"

### 5.1 인력
| 역할 | 인원 | 나와의 관계 |
| --- | --- | --- |
| **기본 챗봇 모델 개발 담당** | **1명 (초급 AI 엔지니어)** | **본 SDP의 대상 = 나** |
| 컨설턴트 챗봇 모델 개발 담당 | 별도 | 그래프 구조 공유 · **market/review/trend 1·2층 소유** · 조회 함수 제공 |
| 서비스 개발 담당 | 별도 | 내 챗봇을 **호출**함 (API 계약) |
| 모델 서빙 담당 | 별도 | SDXL Triton 서빙(2차) |
| 인프라 담당 | 윤승준 | 배포·CI/CD |
| **PM** | **전민재** | 일정·의사결정 · **검증 함수 4종 직접 개발**(§2.4) · **골든 데이터셋·KB 데이터**(§2.7) · **GraphState 규격 주재** |

### 5.2 기술 스택 (초급자 기준으로 의도적으로 단순화)

| 영역 | 선택 | 선택 이유 |
| --- | --- | --- |
| 파이프라인 | **LangGraph** | 파트3에서 학습 완료. 조건부 엣지로 재생성 루프 구현 용이 |
...
| 규제 검증 | **키워드 사전(JSON→DB)** | 법령 API 대비 지연 최소(v9 §14 확정) · **PM 직접 개발** |
| 평가 | **직접 구현(HitRate·MRR·LLM Judge)** | RAGAS 전체는 COULD · **코드는 나, 데이터는 PM** |

### 5.3 개발 환경
- GCP VM(L4 GPU) — VSCode Remote-SSH로 접속 (팀 표준)
- 코드 반영: **GitHub PR → main merge** (VM 직접 수정 금지 — 팀 원칙)
- 브랜치: `feature/model-basic-*` 네이밍
- **파일 소유권**: `graph/`·`nodes/`(내 3층) 는 **내가 소유**, `validation/` 은 **PM**, `cache/`(1·2층) 는 **컨설턴트** → 머지 충돌 방지

### 5.4 ⚠️ OpenAI API 예산 관리 (팀 공통 자원)
- 팀 전체 한도 **$30**, 초과 시 **키 즉시 비활성화**
- 디스코드 팀 채널에서 `!usage`로 사용량 확인 (1분 1회)
- **내 원칙**: 평가 스크립트는 **직렬 실행 + 요청 간 sleep**. 병렬 호출 금지
- 대량 평가 전 **PM에게 사전 공유**하고 실행
- ⚠️ **D10 튜닝일은 재측정을 반복**하므로 호출량이 몰린다. **전날 PM과 잔여 예산 확인** 필수
- ⚠️ **self_check (B) 사실성 검증**은 LLM 재호출이므로, 적용 시 호출량 증가를 PM과 사전 협의
...
[서비스 개발 담당] 사용자 질문 · 사용량/크레딧 확인
        ↓
① 보안1: 입력 검증 (banned_keywords 사전 대조)   ※PM(전민재) 직접 개발
        ↓
  질문 분석 (재작성·정보충분성)  →(정보 부족)→ 보조정보 폼 처리
        ↓
  채널(플랫폼) 확정 → state 저장
        ↓
② 컨텍스트 수집(병렬)   ※ 1·2층=소유자 / 3층 요약 노드=각자 (§2.6)
   · 상권·타겟 분석    → 적재+조회함수=컨설턴트 / ★카피용 요약 노드=나
   · 경쟁사 리뷰 분석  → 전층 컨설턴트 담당(EX-01) / ※기본 챗봇 미사용
   · 업종 벤치마크     → KB 데이터 작성=PM/컨설턴트 / ★인덱싱·검색·요약=나
   · 시즌·이슈 타이밍  → API호출+조회함수=컨설턴트 / ★카피용 요약 노드=나
        ↓
③ ★ 광고 카피 생성 (제안 3안)  ←──┐
        ↓                          │ 품질 미달
   배리언트 랭킹 (LLM Judge) ───────┤
        ↓                          │ 규제 위반
   규제 준수 검증 (키워드 게이트) ──┘   ※PM(전민재) 직접 개발
        ↓
...
⑤ 채널별 리포맷
        ↓
   보안2: 출력 검증                    ※PM(전민재) 직접 개발
        ↓
   self_check (근거·거부 판정)         ※PM(전민재) 직접 개발
        ↓
[서비스 개발 담당] 응답 JSON 전달 → 화면 표시

※ 별도 트랙: 노드는 3층 구조 (§2.6)
   1층 적재 · 2층 조회 함수 · 3층 요약 노드
   [원칙] 1·2층은 같은 사람이 소유, 3층 요약만 각자 만든다
   [내 소유] 벤치마크 인덱싱·검색
   [PM 소유] banned_keywords · 골든데이터셋 · KB 데이터
   [컨설턴트 소유] market_cache · review_cache · 트렌드 API
```

### 6.2 ★ 서비스 개발 담당과의 API 계약 (D1에 반드시 합의)

> 이 계약만 정해지면 **양쪽이 서로를 기다리지 않고 병렬로 개발**할 수 있다.
> 서비스 담당은 이 형식의 Mock으로 화면을 만들고, 나는 이 형식을 채우기만 하면 된다.
...
**차단·거부 시 응답** *(PM 검증 함수의 판정 결과를 그대로 전달)*
```json
{
  "status": "blocked",
  "reason": "exaggeration",
...
### 6.3 ★ 데이터 적재 (담당별 분담 — v1.5 개정)

> **역할 구분**: 테이블(스키마)은 **서비스 개발 담당**이 만들고, **데이터는 담당별로 나눠서 채운다.**
> 채워진 **원본 데이터는 두 챗봇이 함께 읽되**, **읽어서 요약하는 코드는 각자** 만든다.

**[내가 만드는 것 — 코드만]**

| 대상 | 내가 만들 것 | 우선순위 |
| --- | --- | --- |
| 업종 벤치마크 | **FAISS 인덱싱 코드 + 검색 함수 + 요약 노드**<br>※ KB **데이터(JSON 내용)는 PM/컨설턴트 제공** | 🟠 SHOULD (D8) |
| 평가 스크립트 | HitRate·MRR·LLM Judge·CLIP Score **측정 코드**<br>※ **골든 데이터셋 문항은 PM 제공** | 🟠 SHOULD (D9) |

**[PM(전민재)이 제공 — 나는 사용만]**

| 대상 | 내용 | 내가 하는 일 |
| --- | --- | --- |
| `banned_keywords` | 불법·과장·의학 키워드 + 처리방식 + 안내문구 | **직접 접근하지 않음.** PM의 검증 함수만 호출 |
| 골든 데이터셋 55문항 | 질문-정답 쌍 (단일25·다중15·후속5·거부10) | **평가 스크립트에 입력**해 점수 산출 |
| 벤치마크 KB 데이터 | 업종별 우수 카피 레퍼런스 JSON | **인덱싱해서 검색**에 사용 |

**[컨설턴트 챗봇 담당이 적재·조회 — 3층만 내가]**
...
> 그래서 과장 표현은 **block이 아니라 warn(대안 제안)** 으로 처리한다. → **PM 판단 사항**
...
[프롬프트 구성]
   ...
   · 채널 규칙: 인스타(해시태그·이모지) / 블로그(장문·소제목) / 현수막(짧은 한 줄)
...
> ★ **v1.5 재배분으로 확보한 시간 덕분에, D3에 4단계까지 여유 있게 진행 가능**
...
> **초급자 기준으로 의도적으로 얕게** 구성했다. 폴더가 깊으면 파일 찾다가 시간을 쓴다.

```
marketing-ai/
├── validation/                       # ★ PM(전민재) 소유 — 나는 import만
...
│
├── cache/                            # ★ 컨설턴트 담당 소유 (1·2층) — 나는 import만
│   ├── market_repo.py                # market_cache 적재 + get_market() 조회
│   ├── review_repo.py                # review_cache 적재 + 조회 (내가 미사용)
│   └── trend_repo.py                 # 트렌드 API 호출 + get_trend() 조회 ← v1.5
│                                     #   ★ D1에 mock 반환 스텁으로 먼저 커밋
│
├── shared_data/                      # ★ PM/컨설턴트가 채우는 데이터 — 나는 읽기만
│   ├── benchmark_kb.json             # 업종 벤치마크 KB 데이터 (PM/컨설턴트 작성) ← v1.5
│   └── golden_dataset.json           # 골든 데이터셋 55문항 (PM 주도 작성) ← v1.5
│
└── model_basic/                      # ★ 내 작업 영역 (기본 챗봇)
    ├── main.py                       # 챗봇 진입점 (서비스 담당이 호출)
    ├── config.py                     # 모델명·온도·재시도 횟수 등 상수
    ├── llm.py                        # OpenAI 호출 래퍼(+429 백오프)
    │
    ├── graph/                        # ★ LangGraph 골격 (내가 소유)
    │   ├── state.py                  # GraphState 정의(질문·채널·컨텍스트·제안)
    │   └── build_basic.py            # 노드 조립 + 조건부 엣지(재생성 루프)
    │
    ├── nodes/                        # ★ 내가 개발하는 노드들
    │   ├── security_input.py         # 보안1 노드 — PM의 check_input() 감싸기
    ...
    │   ├── regulation_node.py        # 규제검증 노드 — PM의 check_regulation() 감싸기
    │   ├── image_gen.py              # 광고 이미지 생성 노드
    │   ├── image_review.py           # 이미지 자동 검수
    │   ├── channel_format.py         # 채널별 리포맷
    │   ├── security_output.py        # 보안2 노드 — PM의 check_output() 감싸기
    │   └── self_check_node.py        # self_check 노드 — PM의 self_check() 감싸기
    │   # ※ context_review.py 없음 — 경쟁사 리뷰는 컨설턴트 전담(v1.4)
    │   # ※ context_trend.py 없음 — 트렌드 API 호출은 컨설턴트 전담(v1.5)
    │   # ★ 검증 노드 4개는 D2에 모두 연결 완료 — PM이 스텁 내용만 채우면 자동 반영(v1.6)
    │
    ├── clients/                      # ★ 외부 연동부 (교체 가능하게 분리)
    │   └── image_client.py           # 1차 API → 2차 SDXL 교체 지점
    │   # ※ trend_api.py 없음 — cache/trend_repo.py(컨설턴트)로 이관(v1.5)
    │
    ├── retrieval/                    # 벤치마크 RAG (인덱싱·검색 = 내 몫)
    │   ├── vectorstore.py            # FAISS 인덱스 로드·검색
    │   └── embedder.py               # text-embedding-3-small
    │
    ├── scripts/                      # ★ 내 배치 (별도 트랙)
    │   └── build_benchmark_index.py  # KB 데이터(PM 제공) → FAISS 인덱싱
    │   # ※ 데이터 작성 자체는 PM/컨설턴트 영역(v1.5)
    │
    ├── eval/                         # ★ 평가 — 코드는 나, 데이터는 PM
    ...
    │   # ※ golden_dataset.json 은 shared_data/ 에 위치(PM 작성)
    │
    ├── tests/
    │   └── test_pipeline.py          # 그래프 E2E 테스트
    │   # ※ test_regulation.py(검증 함수 단위 테스트)는 PM 영역
    │
    ├── docs/
    │   ├── sdp_model_basic.md        # 본 문서
    │   ├── pipeline_basic_sdp.png    # 파이프라인 다이어그램
    │   ├── api_contract.md           # ★ 서비스 담당과의 API 계약서
...
> ⚠️ **v1.5 개정**: **문항 설계·작성은 전민재 PM 주도**(§2.7). 나는 **측정 코드**를 만들고 결과를 해석한다.
...
> ※ 이 10문항은 **PM의 검증 함수를 검증**하는 용도이므로, PM이 직접 설계하는 것이 자연스럽다.
...
3. "우리 가게가 최고야" 포함 요청 → 규제 검증에 걸려 경고+대안 제안 (PM 함수 연동 확인)
4. 불법 키워드 포함 요청 → 보안1에서 차단 (PM 함수 연동 확인)
5. 정보 부족 질문 → 보조정보 폼 제안
6. 이미지 포함 요청 → base64 이미지 반환
7. 근거(sources) 없는 응답 → self_check가 걸러내는지 확인 (PM 함수 연동 확인)
...
| R3 | **카피 품질이 기대 이하** | 높음 | 중 | **v1.5 재배분으로 확보한 2일을 D3·D10에 재투입**(§2.7) |
| R4 | **API 계약 불일치**(서비스 연동 시) | 중 | 높음 | D1에 계약 문서화(`docs/api_contract.md`) |
| R5 | **캐시 테이블 스키마 미정** | 중 | 중 | D1에 서비스 담당과 협의. 안 되면 **JSON 파일로 우회** |
| R6 | **상권/트렌드 API 키 발급 지연** | 중 | 중 | 컨설턴트 담당 영역. **mock 반환값으로 요약 노드 먼저 개발** |
| R7 | **초급자 LangGraph 난이도** | 중 | 중 | 파트3 학습자료 재활용. **30분 룰** 적용 |
| R8 | **일정 지연** | 중 | 높음 | COULD부터 버린다(§2.3). 8/22 버퍼 확보 |
| R9 | **컨설턴트 담당과 컨텍스트 형식 충돌** | 중 | 중 | **원본 캐시는 공유, 요약 함수는 각자** 원칙을 D1에 합의(§2.2 주의사항 4·6) |
| R10 | **PM 검증 함수 제공 지연**(PM은 조율 업무 병행) | 중 | 중 | **★ 스텁 선제공 원칙**(§2.4·§3.6) — D1에 pass 스텁을 받아 노드를 미리 연결. 함수 도착 시 **내 코드 수정 불필요** |
| R11 | **컨설턴트의 `get_market()`·`get_trend()` 제공 지연** | 중 | 중 | **★ 스텁 선제공 원칙**(§3.6) — D1에 mock 반환 스텁 수령. 함수 도착 시 import만 그대로 |
| R12 | **검증 함수 반환 형식 불일치** | 중 | 중 | D1에 **4개 함수 반환 형식 통일**(§2.4). PM과 문서로 고정 |
| R13 | **PM 업무 과부하로 골든 데이터셋·KB 데이터 지연** | 중 | 중 | **PM은 기준만 정하고 문항 작성은 팀 분담**(§2.7 권고). 지연 시 **소규모(20문항)로 먼저 측정** |
...
| **컨설턴트 챗봇 담당** | GraphState 구조 공유 · **3층 분배 확정** · **`get_market()`·`get_trend()` 시그니처 + mock 스텁 수령**(§2.6·§3.6) | **D1 필수** |
| **PM(전민재)** | **검증 함수 4종 인터페이스 + pass 스텁 수령**(§2.4·§3.6) · **골든 데이터셋 기준·문항**(§2.7) · **벤치마크 KB 데이터** · **GraphState 규격 주재** · OpenAI 사용량 | **D1 필수** / 매일 |
...
| 2 | **캐시 테이블 스키마**(내가 읽을 컬럼) | 서비스 담당과 협의 필요 | **8/10 D1** |
| 3 | **컨텍스트 3층 분배 확정**(§2.6) | 본 SDP v1.5 제안 | **8/10 D1** |
| 4 | **`get_market()`·`get_trend()` 반환 형식**(컨설턴트 제공) | 본 SDP §2.6 초안 | **8/10 D1** |
| 5 | **⚠️ 검증 함수 4종 인터페이스**(§2.4) | **PM과 합의 필요** | **8/10 D1** |
| 6 | **★ 스텁 선제공 원칙 팀 합의**(§3.6) | 본 SDP v1.6 제안 | **8/10 D1** |
| 7 | **골든 데이터셋 작성 주체·기준**(§2.7) | 본 SDP는 **PM 주도 + 팀 분담** 제안 | **8/10 D1** |
| 8 | **벤치마크 KB 데이터 작성 담당**(PM vs 컨설턴트) | 본 SDP v1.5 제안 | **8/10 D1** |
| 9 | **GraphState 공통 규격**(두 챗봇 공유 필드) | **PM 주재 확정 필요** | **8/10 D1** |
...
- [ ] 채널(인스타/블로그/현수막)에 따라 결과가 실제로 달라진다
- [ ] **PM의 검증 함수 4종이 노드로 연결되어 동작한다**
...
- [ ] **컨설턴트의 `get_market()`·`get_trend()` 를 연동해 상권·트렌드가 카피에 반영된다**
- [ ] 서비스 개발 담당의 화면에서 내 챗봇 결과가 표시된다
