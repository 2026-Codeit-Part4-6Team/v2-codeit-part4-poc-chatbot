...
### 1.2 이 문서의 목적
위 서비스에서 **서비스 개발 담당 1명이 맡는 부분만** 떼어내, 10일 안에 완주 가능한 실행 계획으로 만든다.

### 1.3 서비스 개발 담당의 한 줄 정의
> **"챗봇이 잘 돌아갈 수 있는 집(화면·API·DB·과금)을 짓는다. 챗봇의 두뇌는 만들지 않는다."**
...
| 영역 | 내용 |
| --- | --- |
| **프론트엔드(UI/UX)** | Streamlit 화면 8종 + 공통 사이드바 + 에러 배너 3종 |
| **백엔드(REST API)** | FastAPI 라우터 12개 (인증·결제·크레딧·이력) |
| **비즈니스 로직** | **일일 무료 사용량 + 크레딧 차감/롤백 + 생성 상태 관리** ← 가장 중요 |
| **데이터베이스** | SQLAlchemy ORM + SQLite3, 테이블 4개 설계·구현 |
| **모델 연동** | 챗봇 호출 **인터페이스(API 계약)** 정의 + Mock 구현 |
| **통합 테스트** | 회원가입 → 생성 → 과금 → 이력까지 E2E 시나리오 |
...
| LangGraph 파이프라인 내부 노드 구현 | 모델 개발 담당 |
| 프롬프트 엔지니어링 / RAG 검색 / SDXL 이미지 생성 | 모델 개발 담당 |
| 상권 API·트렌드 API·리뷰 크롤링 호출 로직 | 모델 개발 담당 |
| `market_cache` / `review_cache` / `banned_keywords` 데이터 적재 | 모델 개발 담당 |
| Triton 서빙·GPU 최적화 | 모델 서빙 담당 |
| GCP VM 배포·CI/CD 파이프라인 | 인프라 담당(윤승준) |

> ⚠️ **경계에서 헷갈리기 쉬운 것**
> - `generations` **테이블 생성·저장 API는 내 일**이지만, **거기 들어갈 카피 내용을 만드는 건 모델 담당**이다.
> - `banned_keywords` **테이블을 만드는 건 내 일**이지만, **키워드를 채우고 판정 로직을 짜는 건 모델 담당**이다.
> - 대기 화면 **UI는 내 일**, 대기 중 표시할 **문구 100건 작성은 기획(PM)** 이 준비한다.
...
| 등급 | 항목 | 판단 기준 |
| --- | --- | --- |
| 🔴 **MUST** | 로그인/회원가입, 사이드바, 챗봇 화면 2종, 크레딧·무료사용량 로직, DB 4테이블, API 12개, Mock 연동 | **없으면 시연 자체가 불가** |
| 🟠 **SHOULD** | 마이페이지(이력·통계), 크레딧 충전 화면, 동의창, 대기 화면, 결과·승인 화면, 에러 배너 3종 | 있으면 완성도 급상승 |
| 🟢 **COULD** | 재사용/재생성, 사이드바 토글 애니메이션, 컨설팅→광고 연계 버튼, React 전환 | **못 해도 감점 없음** |
...
- **스프린트 1**(8/10~8/14): 뼈대 완성 — DB·API·화면이 Mock으로 끝까지 흐른다
- **스프린트 2**(8/17~8/21): 살 붙이기 — 실제 챗봇 연동 + 부가 화면
- **8/22(토)**: 통합 테스트 + 버퍼

### 3.2 ★ 핵심 전략 — Mock 우선 개발 (이 계획의 생명줄)

> **문제**: 모델 담당의 챗봇이 완성되기를 기다리면, 내 일정이 통째로 남에게 종속된다.
> **해법**: 응답 형식(JSON 계약)만 먼저 합의하고, 가짜 응답으로 전부 만든다.
...
> **실작업일 계산**: 8/10~8/22 중 주말(8/15·8/16·8/22 일부) 제외 → **실작업 10일 + 마지막 토요일 버퍼**
> ※ 2026년 8월 15일(광복절)은 **토요일**이라 평일 손실 없음

### 4.1 스프린트 1 — 뼈대 완성 (8/10 월 ~ 8/14 금)

| 일자 | 작업 내용 | 완료 기준(DoD) |
| --- | --- | --- |
| **D1** 8/10(월) | · **모델 담당과 API 계약(JSON 형식) 합의** ← 최우선<br>· 프로젝트 구조 세팅, SQLAlchemy 모델 4개 작성<br>· DB 초기화 스크립트 | `python init_db.py` 실행 시 테이블 4개 생성됨 |
...
| **D5** 8/14(금) | · Streamlit 로그인/회원가입 화면<br>· 메인 대시보드 + **공통 사이드바**<br>· **스프린트 1 회고(KPT)** | 브라우저에서 로그인 → 대시보드 진입 성공 |

> **스프린트 1 목표(가장 중요)**: **"가짜 응답이지만 끝까지 흐르는 앱"** 을 만든다.
...
| 일자 | 작업 내용 | 완료 기준(DoD) |
| --- | --- | --- |
| **D6** 8/17(월) | · 챗봇 화면 2종(기본/컨설턴트) 채팅 UI<br>· 플랫폼 선택 버튼(인스타/블로그/현수막)<br>· 보조정보 입력 폼(선택사항·건너뛰기) | 채팅으로 Mock 결과가 화면에 표시됨 |
| **D7** 8/18(화) | · **★ Mock → 실제 챗봇 교체**(모델 담당과 연동)<br>· 응답 형식 불일치 대응<br>· 통합 오류 처리 | 실제 챗봇 응답이 화면에 표시됨 |
...
| 마일스톤 | 날짜 | 판정 기준 |
| --- | --- | --- |
| **M1** API 계약 확정 | 8/10 | 모델 담당과 JSON 형식 합의 완료 |
...
| 모델 개발 담당 | 별도 | 챗봇 엔진 — 나는 호출만 |
| 인프라 담당 | 별도(윤승준) | 배포·CI/CD |
| PM | 전민재 | 일정·의사결정 |
...
| 영역 | 선택 | 선택 이유 |
| --- | --- | --- |
| 프론트엔드 | **Streamlit** | 코드잇 가이드가 명시적으로 허용. React 대비 학습·구현 시간 1/3. **10일 안에 완주 가능** |
| 백엔드 | **FastAPI** | 파트4-3에서 학습 완료. 자동 문서(/docs)로 모델 담당과 협업 편함 |
...
> **[결정 필요] React 전환 여부**
> 팀 PR에 "추후 React UI로 교체" 계획이 있었다. 그러나 **초급 1명 · 10일** 조건에서 React+FastAPI 신규 구축은
> 완주 위험이 크다. **본 SDP는 Streamlit을 기본안으로 하고, React는 COULD(시간 여유 시)로 둔다.**
...
### 6.1 파이프라인 (서비스 개발 관점)
![파이프라인](pipeline_sdp.png)

```
① 프론트엔드(내 담당)  Streamlit 화면 8종
        ↓
...
- **모델 담당 소유(3개)**: `market_cache`, `review_cache`, `banned_keywords`
  → 나는 **스키마만 정의**해두고 데이터는 채우지 않는다
- **관계**: `users(1) — payments(N)`, `users(1) — generations(N)`

### 6.3 ★ 모델 담당과의 API 계약 (D1에 반드시 합의)
...
| 9 | GET | `/generations/{id}` | 이력 상세(재사용용) | 🟢 COULD |
...
> **이 로직이 이 프로젝트에서 내가 만드는 가장 중요한 코드다.** D3에 집중해서 구현한다.
...
│   └── api_contract.md          # ★ 모델 담당과의 API 계약서
│
├── docker-compose.yml
├── .env.example
└── README.md
...
- API는 `/docs`(Swagger)로 자동 문서화 → 모델 담당이 스스로 확인 가능
...
| R5 | **Streamlit UI 한계**(스토리보드 재현 어려움) | 중 | 낮음 | 기능 우선, 디자인은 근사치. `st.columns`·`st.container`로 최대한 재현 |
| R6 | **일정 지연** | 중 | 높음 | COULD 항목부터 버린다(§2.3). 8/22 버퍼 확보 |
| R7 | **OpenAI 429 오류** | 중 | 중 | 서비스 개발자는 **재시도·에러 배너 UI만** 담당. 호출 제어는 모델 담당 |

> **가장 큰 리스크는 R1이다.** 그래서 이 SDP 전체가 **Mock 우선**으로 설계돼 있다.
...
| 6 | 재생성 시 동의창 재노출 여부 | 미정 | 8/19 D8 |
| 7 | React 전환 여부 | Streamlit 기본안 | 8/17 D6 |
| 8 | 서비스명 통일 (마케팅AI) | 문서마다 상이 | 8/10 D1 |
...
**서비스 개발 담당과의 관계**: 골든 데이터셋 **구축·평가는 모델 개발 담당의 업무**다.
다만 서비스 개발자는 **평가 결과를 저장·조회할 수 있는 구조**(generations 테이블의 tokens_used 등)를 제공한다.
