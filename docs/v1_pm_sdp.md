...
| **업종 벤치마크 KB 데이터** | 우수 카피 레퍼런스 JSON (컨설턴트 모델 개발 담당자와 분담) |
...
> 3. **벤치마크는 반으로 갈린다**: **데이터 작성=전민재 PM(또는 컨설턴트 모델 개발 담당자)**, **인덱싱·검색 코드=기본 챗봇 모델 개발 담당자**
> 4. **골든 데이터셋도 갈린다**: **기준 설계·검수=전민재 PM**, **문항 작성=팀 분담**
> 5. **한 파일에는 관리자 한 명**: `validation/`은 **전민재 PM 관리**한다. 두 챗봇 모델 개발 담당자는 **import만** 하고 내부를 수정하지 않는다
...
- **파일 소유권**: `validation/`·`shared_data/` 는 **전민재 PM 관리**.
...
│
├── shared_data/                      # ★ 전민재 PM이 채우는 데이터 자산
│   ├── legal_kb.md                   # ★ 규제 RAG 원본 (전민재 PM 작성·1차 검토, 2차 검토 - 다른 담당자)
│   ├── legal_kb.json                 # 스크립트가 생성 (gitignore 가능)
│   ├── legal_index/                  # FAISS 인덱스 파일
│   ├── golden_dataset.json           # 골든 데이터셋 (기준=PM, 문항=팀 분담)
│   └── benchmark_kb.json             # 업종 벤치마크 KB (컨설턴트 챗봇 모델 개발 담당자와 분담)
│
├── scripts/                          # ★ 전민재 PM 배치 스크립트
...
> **다른 담당자 관리 영역**(참고 — 전민재 PM은 수정하지 않음)
> ```
> ├── backend/              # 서비스 개발 담당자 관리
> |   ├── model_basic/      # 기본 챗봇 모델 개발 담당자 관리
> |   ├── model_consult/    # 컨설턴트 챗봇 모델 개발 담당자 관리
> |   ├── cache/            # 컨설턴트 챗봇 모델 개발 담당자 관리(1·2층)
> └── frontend/             # 서비스 개발 담당자 관리
> ```
