...
### 1.2 이 문서의 목적
위 서비스에서 **전민재 PM이 직접 맡는 부분만** 떼어내, 10일 안에 완주 가능한 실행 계획으로 만든다.
**PM은 조율이 1순위이고 개발은 그와 병행 가능한 범위로 한정**한다.

### 1.3 전민재 PM의 한 줄 정의
> **"팀이 막히지 않게 하는 것이 1순위다.
> 그리고 두 챗봇이 공통으로 쓰는 '안전장치(검증 함수 4종)'와 그 재료(데이터 자산)를 만든다."**

### 1.4 ★ PM 담당 범위가 어떻게 정해졌나 (역산 도출)

> 세 담당자 SDP(`sd_sdp.md`·`basic_sdp.md`·`consult_sdp.md`)를 대조해
> **다른 세 명이 하지 않는 업무**만 남긴 결과가 본 SDP의 범위다.
...
1. **"무엇을 차단할까"는 코딩이 아니라 정책 판단**이다. 코드잇 가이드의 PM 역할에도 *"서비스 기획"*, *"서비스 테스트 및 평가"* 가 명시돼 있다.
2. **입력=문자열, 출력=판정**이라 로직이 독립적이다. → **조율 업무와 30분 단위로 병행 가능**하다.
3. **두 챗봇이 같은 함수를 공유**한다. 어느 한쪽 담당자가 만들면 다른 쪽이 요청·대기하게 되므로, **중립적 위치인 PM이 적합**하다.

> ⚠️ **반대로 PM이 맡으면 안 되는 것**: 카피 생성 노드·LangGraph 그래프 조립처럼
> **매일 손대야 하는 작업**은 PM이 병목이 된다. 그래서 본 SDP에서 제외했다.
...
> **본 SDP는 컨설턴트 챗봇 담당자·기본 챗봇 담당자·서비스 개발 담당자가 하지 않는 업무만 다룬다.**

### 2.1 ✅ 포함 범위 (전민재 PM이 직접 한다)
...
| **골든 데이터셋 기준 설계·검수** | 유형별 문항 수 · 정답 판정 기준 확정<br>※ **문항 작성은 팀 분담**, PM은 기준 수립 + 최종 검수 |
| **업종 벤치마크 KB 데이터** | 우수 카피 레퍼런스 JSON (컨설턴트 담당자와 분담) |
...
| **업종 벤치마크 인덱싱·검색 코드**(FAISS) | 기본 챗봇 담당자 ※ **데이터 작성만 PM** |
| **`build_basic.py` 그래프 조립** | 기본 챗봇 담당자 |
| **검증 함수를 LangGraph 노드로 감싸 연결** | **각 챗봇 담당자** ※ PM은 **함수만 제공** |
...
| **프론트엔드 화면(Streamlit UI)** | 서비스 개발 담당자 |
| **REST API 라우터 · 세션 · 로그인** | 서비스 개발 담당자 |
| **DB 테이블 스키마 설계(DDL)** | 서비스 개발 담당자 ※ PM은 `banned_keywords` **데이터만** 적재 |
...
> 1. **함수와 노드는 다르다**: PM은 **판정 함수**를 만들고, **LangGraph 노드로 감싸는 건 각 챗봇 담당자**다
> 2. **사전 데이터와 DB 테이블은 다르다**: `banned_keywords` **테이블 DDL은 서비스 담당**, **데이터 적재는 PM**이다
> 3. **벤치마크는 반으로 갈린다**: **데이터 작성=PM(또는 컨설턴트)**, **인덱싱·검색 코드=기본 챗봇 담당자**
> 4. **골든 데이터셋도 갈린다**: **기준 설계·검수=PM**, **문항 작성=팀 분담**
> 5. **한 파일에는 주인이 한 명**: `validation/`은 **PM 소유**다. 두 챗봇 담당자는 **import만** 하고 내부를 수정하지 않는다
...
| 등급 | 항목 | 판단 기준 |
| --- | --- | --- |
| 🔴 **MUST** | **검증 함수 4종 스텁 D1 제공**, `banned_keywords` 사전, `check_input`·`check_output` 실구현, GraphState 규격 주재, **수치 3건 확정**, OpenAI 예산 관리, 데일리 스크럼 | **없으면 두 챗봇 담당자가 멈춘다** |
| 🟠 **SHOULD** | **`check_regulation` RAG 전환**(legal_kb 30~50청크), `self_check` (A)규칙기반, 골든 데이터셋 기준·검수, 벤치마크 KB 데이터, 통합 테스트 총괄 | 있으면 완성도·평가점수 급상승 |
| 🟢 **COULD** | `self_check` (B)사실성 검증(LLM 재호출), legal_kb 청크 확장(50→100), 프롬프트 인젝션 패턴 고도화 | **못 해도 감점 없음** |

> **`check_regulation` RAG가 SHOULD인 이유**: 키워드 방식으로도 **동작은 한다.**
> RAG는 품질 향상이므로, 시간이 부족하면 **1단 키워드만으로 마감**하고 발표에서 *"2단 RAG는 향후 과제"* 로 설명한다.

### 2.4 ★ 검증 함수 4종 — 인터페이스 (D1 최우선 제공)

> **반환 형식을 통일**해야 두 챗봇 담당자가 **같은 방식으로 노드에 감쌀 수 있다.**
...
> 규제 검증은 **"왜 안 되는지(law·reason) + 대안은 뭔지(alternative)"** 를 줘야 사용자가 다음 행동을 할 수 있다.
> 이것이 §6.2에서 RAG로 전환하는 핵심 이유다.

### 2.5 ★ 스텁 선제공 원칙 — D1 오전에 반드시

> **문제**: 다른 담당자 SDP는 **8/10 D1에 PM 스텁을 기다린다.**
...
> **효과**: 두 챗봇 담당자는 **함수가 미완성이어도 import해서 노드를 연결**할 수 있다.
> PM이 이후 **내용만 채우면 상대 코드는 손댈 필요가 없다.**
> → **R1(PM 지연으로 팀 대기) 리스크가 구조적으로 해소**된다.
...
### 3.2 ★ 핵심 전략 — "조율 우선, 개발은 잘게" (PM 전용 원칙)

> **문제**: PM이 코드에 몰입하면 **팀 조율이 멈춘다.** PM 1명이 막히면 **세 명이 동시에 막힌다.**
...
**왜 검증 함수가 PM 업무에 맞는가**: 입력이 문자열 하나, 출력이 판정 결과 하나다.
**긴 집중이 필요 없어** 회의 사이사이에 진행할 수 있다.
반대로 카피 생성 프롬프트 튜닝처럼 **몇 시간 몰입이 필요한 작업**은 PM이 맡으면 진도가 안 나간다.
...
### 3.4 데일리 스크럼 주재 (PM이 진행)
매일 10~15분 **[어제 한 일 / 오늘 할 일 / 막히는 부분(Blocker)]**.
**PM의 역할은 발언이 아니라 블로커를 받아 해소 경로를 정하는 것**이다.
...
PM이 늦게 올릴수록 상대가 오래 스텁 상태로 개발하게 된다.

---

## 4. 일정 및 마일스톤 — "언제까지 무엇을 해내야 하는가?"

> **실작업일 계산**: 8/11~8/22 중 주말 제외 → **실작업 10일 + 8/22(토) 버퍼**
...
| 일자 | 작업 내용 | 완료 기준(DoD) |
| --- | --- | --- |
| **D1** 8/11(화) | **[오전·최우선]**<br>· **★ 검증 함수 4종 `pass` 스텁 커밋**(§2.5) → 두 담당자에게 즉시 통보<br>· **GraphState 공통 규격 주재·확정**(두 챗봇 공유 필드)<br>· **★ 미확정 수치 3건 확정** — 일일 무료 횟수 / 회당 차감액 / 잔액 단일 소스<br>**[오후]**<br>· `validation/` 디렉토리 구조 세팅<br>· 골든 데이터셋 기준 초안(유형별 문항 수·판정 기준) | **두 담당자가 4종 함수를 import 가능**<br>수치 3건이 config에 반영됨 |
...
| **D6** 8/18(화) | · `build_legal_kb.py` — md → JSON 파싱 → 임베딩 → FAISS 인덱싱<br>· **임베딩 텍스트에 위반 예시 포함**(§6.3 핵심)<br>· 검색 테스트: "우리 가게가 최고" → 표시광고법 제3조 검색되는지 | 구어체 카피로 조문이 정확히 검색됨 |
| **D7** 8/19(수) | · **★ `check_regulation()` 2단 RAG 판정 연결**<br>· 1단 통과 시 RAG 생략(지연 방어) · 걸리면 RAG + LLM 판정<br>· `law`·`reason`·`alternative` 필드 채우기<br>· 기본 챗봇 담당자와 **연동 확인** | "최고" 포함 카피 → 조문 근거 + 대안 문구 반환 |
| **D8** 8/20(목) | · **골든 데이터셋 검수**(팀이 작성한 문항 최종 확인)<br>· **거부 문항으로 검증 함수 4종 역검증**<br>· 과잉 차단·누락 케이스 보정<br>· **업종 벤치마크 KB 데이터 작성**(컨설턴트와 분담) | 거부 문항 정확도 측정 완료 |
| **D9** 8/21(금) | · `self_check()` (B)사실성 검증 [COULD, 예산 여유 시]<br>· 검증 함수 튜닝(오탐 감소)<br>· **스프린트 2 회고(KPT) 주재**<br>· **발표 자료 초안** — 검증 설계 근거 정리 | 튜닝 전후 오탐률 비교표 |
...
| 마일스톤 | 날짜 | 판정 기준 |
| --- | --- | --- |
| **M1** 스텁 4종 제공 + 규격·수치 확정 | 8/11 | **두 담당자가 함수를 import 가능**, 수치 3건 config 반영 |
...
| 기본 챗봇(게시물 생성) 담당 | 별도 | **검증 함수 4종 전부** 노드로 연결 |
| 컨설턴트 챗봇 담당 | 별도 | **검증 함수 3종**(보안1·보안2·self_check) 노드로 연결 |
| 모델 서빙 담당 | 별도 | SDXL Triton (PM 범위 아님) |
| 인프라 담당 | 윤승준 | 배포·CI/CD (PM 범위 아님) |
...
| 규제 RAG | **text-embedding-3-small + FAISS** | 기본 챗봇 담당자의 벤치마크 RAG와 **같은 스택 재사용** |
...
- **파일 소유권**: `validation/`·`shared_data/` 는 **PM 소유**.
  두 챗봇 담당자는 **import만** 하고 내부를 수정하지 않는다 → 머지 충돌 방지

### 5.4 ⚠️ OpenAI $30 예산 관리 (PM이 직접 관리)

> **PM은 팀 전체 예산의 관리자다.** 초과 시 **키가 즉시 비활성화되어 팀 전체가 멈춘다.**

| 항목 | 관리 방법 |
| --- | --- |
| 사용량 확인 | 디스코드 팀 채널에서 `!usage` (1분 1회) — **매일 스크럼 때 공유** |
| 대량 평가 승인 | 담당자가 골든 데이터셋 평가를 돌리기 전 **PM에게 사전 공유** |
| 내 소비 지점 | `check_regulation` 2단 RAG · `self_check` (B) — **둘 다 LLM 호출** |
...
### 6.1 파이프라인 (전민재 PM 관점)
![파이프라인](images/pipeline_pm_sdp.png)
...
> **강사님 피드백 반영**: *"해당 파트는 RAG로 진행하면 어떨까"*
> **검토 결과: 규제 준수 검증만 RAG로 전환하고, 보안1·보안2는 키워드를 유지한다.**
...
```
**소상공인은 법을 모른다.** *왜 안 되는지, 뭘로 바꿔야 하는지* 알려주는 게 서비스의 진짜 가치다.

**③ 이미 팀에 RAG 인프라가 있어 추가 비용이 작다**
기본 챗봇 담당자가 **업종 벤치마크 RAG**(FAISS + text-embedding-3-small)를 만든다.
규제 KB를 **같은 스택으로 컬렉션만 추가**하면 되므로 새 인프라가 필요 없다.

#### 6.2.2 보안1·보안2는 왜 키워드를 유지하나

| 이유 | 설명 |
| --- | --- |
| **판정이 이분법적** | "마약 광고 만들어줘"는 **맥락을 볼 것도 없이 차단**. 조문 검색·LLM 추론이 불필요 |
| **비용 절감 지점** | 보안1은 **카피 생성 전**이라 여기서 막으면 **LLM 호출 0회**. RAG로 바꾸면 차단하려고 LLM을 부르는 모순 |
...
> **`.csv`가 아니라 `.md`인 이유**: 조문은 **길고 줄바꿈이 많다.**
> CSV 셀에 넣으면 따옴표 이스케이프가 지저분해지고 편집 실수가 잦다.
> `.md`는 **GitHub에서 바로 읽혀** 팀원 검토도 쉽다.
...
> **흔한 실수**: 조문 전체를 통째로 임베딩하면 검색이 잘 안 된다.
> **이유**: 사용자 카피는 *"우리 가게가 최고예요"* 같은 **구어체**인데,
> 조문은 *"객관적 근거 없이 자기 상품이 다른 사업자의 것보다 우량..."* 같은 **법률 문어체**다. 벡터 거리가 멀다.
...
> **조문 복사보다 예시 작성이 더 중요하다.** 조문은 법제처(open.law.go.kr)에서 가져오면 되지만,
...
> **답**: **RAG 검색 자체는 빠르다.** 문제는 LLM을 한 번 더 부르는 것이다.
...
**해법: 정상 카피는 LLM을 부르지 않게 한다.**

```
[1단] 키워드 1차 스크리닝  (수 ms, LLM 호출 0회)
   · banned_keywords·RISKY_HINTS에 하나도 안 걸리면 → 즉시 통과
   · 대부분의 정상 카피가 여기서 끝남 (전체의 70~80% 추정)
        ↓ (걸린 경우만)
[2단] RAG 정밀 판정  (검색 수십 ms + LLM 2~5초)
   · legal_kb 검색 → 관련 조문 3개
   · LLM이 "맥락상 위반인가?" 판단 + 대안 문구 생성
```

```python
def check_regulation(copy: str) -> dict:
    # [1단] 키워드 스크리닝 — LLM 호출 없음
...
```
marketing-ai/
├── validation/                       # ★★ 전민재 PM 소유 — 두 챗봇은 import만
...
│   └── data/
│       └── banned_keywords.csv       # 규제 키워드 사전 (PM 작성)
│
├── shared_data/                      # ★ PM이 채우는 데이터 자산
│   ├── legal_kb.md                   # ★ 규제 RAG 원본 (사람이 작성·검토)
│   ├── legal_kb.json                 # 스크립트가 생성 (gitignore 가능)
│   ├── legal_index/                  # FAISS 인덱스 파일
│   ├── golden_dataset.json           # 골든 데이터셋 (기준=PM, 문항=팀 분담)
│   └── benchmark_kb.json             # 업종 벤치마크 KB (컨설턴트와 분담)
│
├── scripts/                          # ★ PM 배치 스크립트
...
│
├── tests/                            # ★ PM 단위 테스트
│   ├── test_security.py              # 보안1·2 차단 테스트
│   ├── test_regulation.py            # 규제 검증 (과잉 차단·누락 케이스)
│   └── test_self_check.py            # 답변 검증 규칙 테스트
│
├── docs/                             # ★ PM 총괄 문서
│   ├── sdp_pm.md                     # 본 문서
│   ├── pipeline_pm_sdp.png           # 파이프라인 다이어그램
│   ├── validation_contract.md        # ★ 검증 함수 4종 계약서(두 담당자용)
│   ├── graphstate_spec.md            # ★ GraphState 공통 규격(D1 확정)
│   ├── decisions.md                  # ★ 의사결정 로그(수치 3건 등)
...
> **다른 담당자 영역**(참고 — PM은 수정하지 않음)
> ```
> ├── model_basic/      # 기본 챗봇 담당자 소유
> ├── model_consult/    # 컨설턴트 챗봇 담당자 소유
> ├── cache/            # 컨설턴트 챗봇 담당자 소유 (1·2층)
> ├── backend/          # 서비스 개발 담당자 소유
> └── frontend/         # 서비스 개발 담당자 소유
> ```
...
| 작업 | 담당 | 근거 |
| --- | --- | --- |
| **기준 설계**(유형별 문항 수·정답 판정 기준) | **전민재 PM** | *"무엇이 좋은 답인가"* 를 정하는 것은 **서비스 기획 판단**. 코드잇 가이드 PM 역할 *"서비스 테스트 및 평가"* 에 해당 |
| **문항 작성** | **팀 분담**(각 15~20문항) | PM이 전부 쓰면 조율 업무가 멈춘다 |
| **최종 검수** | **전민재 PM** | 편향·중복 제거, 난이도 균형 |
| **거부 문항 설계** | **전민재 PM** | **PM이 만든 검증 함수를 역검증**하는 용도라 PM 영역 |

> **왜 만든 사람이 자기 문항을 짜면 안 되는가**: 자기가 잘 만든 것 위주로 문항을 구성하게 되어
...
| 영역 | 문항 수 | 작성자 |
| --- | --- | --- |
| 기본 챗봇(카피 생성) | 25 | 기본 챗봇 담당자 |
| 컨설턴트(전략 제안) | 20 | 컨설턴트 담당자 |
| **거부(규제·보안)** | **10** | **전민재 PM** |
| **합계** | **55** | — |

### 8.2 ★ 거부 문항 — PM의 검증 함수를 역검증
...
> **발표용**: *"1단 통과율 78%로, 정상 카피는 LLM 호출 없이 통과시켜 응답 지연과 비용을 방어했습니다."*
> 이런 수치가 있으면 설계 판단이 근거로 인정받는다.

### 8.4 통합 테스트 시나리오 총괄 (8/22, PM 주재)

```
[PM 검증 함수 확인]
1. "마약 판매 광고 만들어줘"          → 보안1에서 block, LLM 호출 0회 확인
...
9. 컨설턴트: 전략 제안 → [이 전략으로 광고 만들기] → 기본 챗봇 연동
...
| # | 리스크 | 발생 확률 | 영향 | 대응책 |
| --- | --- | --- | --- | --- |
| **R1** | **★ PM 함수 지연 → 두 챗봇 담당자 동시 대기** | 중 | **치명적** | **스텁 선제공 원칙**(§2.5) — D1 오전에 `pass` 스텁 4종 커밋. 구조적으로 해소 |
| **R2** | **★ PM이 개발에 몰입해 조율이 멈춤** | **높음** | **치명적** | **§3.2 오전 조율 / 오후 개발 블록 분리.** PM 1명이 막히면 3명이 막힌다 |
| R3 | **OpenAI $30 한도 초과 → 키 비활성화** | 중 | **치명적** | PM이 직접 관리(§5.4). 매일 스크럼에서 `!usage` 공유. 2단 게이트로 호출 절감 |
...
| R7 | **`banned_keywords` 테이블 DDL 지연**(서비스 담당) | 중 | 낮음 | **CSV 파일로 먼저 동작**시키고, 테이블 생성 후 적재로 전환 |
| R8 | **골든 데이터셋 문항 작성 지연**(팀 분담) | 중 | 중 | PM은 **기준만 D1에 배포**. 지연 시 **거부 10문항만으로 먼저 측정** |
| R9 | **초급자 RAG 구현 난이도** | 중 | 중 | 기본 챗봇 담당자의 **벤치마크 RAG 코드를 참고**(같은 스택). 30분 룰 적용 |
| R10 | **일정 지연** | 중 | 높음 | COULD부터 버린다(§2.3). **RAG 전환은 SHOULD** — 안 되면 키워드로 마감 |

> **가장 큰 리스크는 R1과 R2다.**
> R1은 스텁 선제공으로, R2는 시간 블록 분리로 막는다. 둘 다 **사전에 정한 규칙**으로만 막을 수 있다.
> **PM의 실패는 개인의 실패가 아니라 팀 전체의 지연**이라는 점을 항상 인식한다.

---

## 10. 팀 협업 규칙 (PM이 주재)

### 10.1 데일리 스크럼 (매일 10~15분, PM 진행)
[어제 한 일 / 오늘 할 일 / 막히는 부분(Blocker)] 공유
**PM의 역할**: 발언이 아니라 **블로커를 받아 해소 경로를 정하는 것**

### 10.2 스프린트 회고 — KPT (PM 진행)
| 항목 | 내용 |
| --- | --- |
| **Keep** | 좋았던 점, 계속 유지할 점 |
...
> **PM은 팀원 협업일지도 주기적으로 검수**해 작성 누락을 방지한다.

### 10.4 담당자별 인터페이스 (PM 기준)

| 상대 | PM이 제공하는 것 | 시점 |
| --- | --- | --- |
| **기본 챗봇 담당자** | 검증 함수 **4종**(보안1·규제검증·보안2·self_check) + GraphState 규격 | **D1 스텁 / D2~D7 실구현** |
| **컨설턴트 챗봇 담당자** | 검증 함수 **3종**(보안1·보안2·self_check) + GraphState 규격 | **D1 스텁 / D2~D4 실구현** |
...
> PM이 내린 결정과 **그 근거**를 기록한다. 발표에서 그대로 쓰인다.
...
> ⚠️ **1~3번은 PM이 직접 결정해야 하는 사안**이다. 다른 담당자는 PM 결정을 기다린다.
...
| 5 | **GraphState 공통 규격**(두 챗봇 공유 필드) | PM 주재 확정 필요 | **8/11 D1** |
| 6 | **`banned_keywords` 테이블 DDL** | 서비스 담당에게 요청 | **8/11 D1** |
| 7 | **골든 데이터셋 문항 분담**(누가 몇 개) | 본 SDP는 25/20/10 제안 | **8/11 D1** |
| 8 | 과장 표현 처리(block vs warn) | 본 SDP는 **warn** 제안 | 8/13 D3 |
| 9 | 컨설턴트 챗봇 과금 여부 | 미정 | 8/12 D2 |
...
- [ ] **★ `check_regulation()` 이 조문 근거(`law`)와 대안 문구(`alternative`)를 반환한다**
...
또 저희 서비스는 차단이 목적이 아니라 '왜 안 되는지와 대안'을 알려주는 것이 목적이라,
조문 근거를 검색해 제시할 필요가 있었습니다.

지연을 막기 위해 2단 게이트로 설계했습니다. 1단 키워드 스크리닝에서
정상 카피 78%가 LLM 호출 없이 통과하고, 걸린 것만 RAG 판정으로 넘어갑니다.
덕분에 평균 응답 시간 증가 없이 판정 품질을 확보했습니다.

반면 보안 검증은 키워드를 유지했습니다. 불법 품목은 맥락 판단이 불필요하고,
입력 단계에서 걸러야 LLM 비용이 발생하지 않기 때문입니다."
```
...
- **기본 챗봇 담당 SDP** (`basic_sdp.md`) — 검증 함수 4종 수요처
- **컨설턴트 챗봇 담당 SDP** (`consult_sdp.md`) — 검증 함수 3종 수요처
- 팀 통합 파이프라인 (`pipeline_v10.png`) · ERD (`erd_v10.png`)
- 파이프라인 다이어그램 (`pipeline_pm_sdp.png`)
