...
| 역할 | 먼저 볼 곳 |
| --- | --- |
| 처음 합류 | §1 문서 개요 → §2.1 두 챗봇 → §9 화면 목록 → §7 파이프라인 |
| 백엔드/API | §3 기능 요구사항 → §5 데이터 요구사항 → **§14 규제검증(키워드)** → §10 과금 |
| 모델 개발 | §4 비기능 → §7 파이프라인 → §9.3 평가 지표 |
| 인프라/배포 | §4 비기능(배포·429) → §8 디렉토리 구조 |
| 프론트엔드 | §2.2 사용자 흐름 → §9 화면 목록 → §6.1 UI → §10 과금 |

---

## 🔧 v8 → v9 수정·추가 이력 — 무엇을 왜 바꿨는가

> 추가 요구사항(add2)과 스토리보드를 반영했다. **v8의 나머지 내용은 그대로다.**

### [수정 1] 규제·보안 검증: "법령 API + RAG" → **경량 키워드 사전 방식** (add2 핵심)
- **v8의 방식**: FR-21 규제 검증을 "표시광고법·식품표시광고법 **RAG 게이트**"로,
  보안1도 일반적 차단으로 서술 → 실제 구현은 **국가법령정보 OPEN API 호출 → 데이터 정제(파싱) → LLM 추론** 경로를 암시했다.
- **왜 바꾸나(add2의 지적)**: 이 경로는 **호출·정제·추론이 겹겹이라 지연이 크다.**
...
### [추가 2] 회원 위치 데이터를 "상세하게" 저장 (add2 · 스토리보드 S-02/S-04)
...
### [추가 3] 재사용/재생성을 위한 "전체 이력 저장" + 데이터 보관정책 (add2 · S-08)
...
> 여기에 add2가 요구한 **정책 결정 3건**이 추가됐다. 전부 §13 체크리스트에 모았다.
...
| FR-19 | 보안1: 입력 검증 노드 | 사용자 질의 | 프롬프트 인젝션·탈옥 차단, **banned_keywords 사전 대조로 불법 키워드 차단/재질문** | add 16 + add2 |
| FR-20 | 보안2: 출력 검증 노드 | 생성 답변 | 민감정보·신상·시스템 프롬프트 누출 차단 | add 16 |
| FR-21 | 규제 준수 검증 노드 (**키워드 사전 기반**) | 생성 카피 | 금지 표현('최고'·'유일' 등)·불법 품목 키워드 판정 + 재질문/생성 거부 안내 | add 16 + add2 |
...
│   └── reports/                    # 평가 결과 리포트
├── docs/
│   ├── v9_sw_req_spec.md           # 본 문서(README)
│   ├── erd_v9.png                  # ★ERD 다이어그램
│   ├── pipeline_v9.png             # ★파이프라인 다이어그램
│   └── 보고서.pdf                   # 최종 제출 보고서
├── scripts/
...
## 14. (v9 신규) 규제·보안 검증 구현 상세 — 키워드 사전 방식 (add2 반영)

> add2가 "법령 OPEN API + 정제 + LLM 추론은 너무 무겁다"고 지적했다. 그 대안을 구체화한다.
...
### 14.2 banned_keywords 테이블 (기본 채택 형식)
| 컬럼 | 타입 | 설명 |
| --- | --- | --- |
...
## 16. (v9 신규) 데이터 보관 정책 (add2 반영 · FR-36)
...
## 17. (v9) 오늘 팀에서 확정할 것 — 통합 체크리스트

### 🔴 개발 차단 이슈 (먼저 확정)
- [ ] **일일 무료 한도** 5회 vs 10회 (스토리보드 수치 모순 #1)
- [ ] **회당 크레딧 차감액** 10 vs 50, 문구/이미지 차등 여부 (#2)
- [ ] **크레딧 잔액 단일 소스** 확정 — 전 화면 동기화 (#3)
