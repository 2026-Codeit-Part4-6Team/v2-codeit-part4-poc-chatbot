...
| 함수 | 필드 | 타입 | 필수 | 설명 |
| --- | --- | --- | --- | --- |
| 공통 | `action` | string | O | `pass` \| `warn` \| `block` |
| 공통 | `message` | string | O | 사용자 표시 문구. `pass`일 때 `""` |
| **`check_output`만** | `masked_text` | string | O | **`pass`면 입력 원문 그대로**, `warn`이면 마스킹본, `block`이면 `""` |

> ★ **`masked_text` 가 없으면 §6.3의 마스킹 규칙은 구현할 수 없다.**
> 노드는 "민감정보가 있다"는 사실만 알고 *어디가* 민감정보인지 모르기 때문이다.
> 마스킹 규칙을 `validation/` 이 관리하는 이유는 §13-③과 같다 —
> 두 챗봇이 각자 정규식을 만들면 가리는 범위가 달라진다.
...
> D5 프론트 작업 시 v0.3 계약 변경(D-046 절차)으로 검토한다.
...
```
1차 요청 → 차감 → check_input warn / question_analysis 정보부족
...
> 서비스는 **모델 호출 전에 차감**하므로(§9.1) 실제 구현은 **롤백**이다. 결과는 같다.
>
...
- 마스킹 규칙은 `validation/` 이 관리한다 — 두 챗봇이 각자 정규식을 만들면
...
| `check_output` | 스텁(`pass`) | **8/19** | `masked_text` 채워짐, 누출 탐지 시 `warn`/`block` |
...
| `insufficient_info` | `question_analysis` 정보 부족 | **재질문 입력창** → `extra.clarification` |
...
| --- | --- | --- | --- | --- |
| V-01 | `check_input` `warn` 재질문 UX | **확정** — 그래프 종료 → `action="reask"`, `reason="need_clarification"` → 프론트가 직전 요청을 유지한 채 `extra.clarification` 추가 + **새 Idempotency-Key** 로 재호출(v0.3 §6.5) | 전민재 PM | 8/18 종료 |
...
| V-03 | `self_check` `reject` 재시도 횟수 | **확정** — **1회**, 카운터는 `self_check_retry_count` 로 분리(D-156)(규제 재생성과 공유 금지) | 전민재 PM | 8/18 종료 |
