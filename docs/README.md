...
> 공개 전 `YOUTUBE_VIDEO_ID`를 실제 영상 ID로 교체합니다.
...
외부 상권·트렌드 API 대신 문항별 고정 fixture를 주입하고, 생성 모델과 Judge 모델을 분리해 같은 조건에서 답변을 비교했습니다.
...
RAGAS 종합 점수는 목표 0.70에 미달했습니다. 고정 `top-k=3` 검색에서 불필요한 조문이 함께 반환되어 `context_precision`이 0.3871로 낮아진 것이 주요 원인이었습니다.
...
| 이름 | 역할 | 주요 업무 |
| --- | --- | --- |
| 전민재 | PM·검증 | 일정·의사결정, 공용 계약, 검증 함수 4종 |
| 조희원 | 기본 챗봇 모델 | 광고 문구·이미지 생성 LangGraph |
| 김재헌 | 컨설턴트 챗봇 모델 | 상권·트렌드 데이터 계층, 전략 생성 LangGraph, 평가 |
| 윤승준 | 서비스 개발 | React·FastAPI 서비스, 인증·크레딧·생성 이력, 배포 |

### 컨설턴트 챗봇 담당 상세

- 상권·검색 트렌드 API와 Cache-Aside 데이터 계층 구현
- 구체적인 상품·메뉴가 있을 때만 트렌드를 조회하는 조건부 경로 설계
- 상권·트렌드·계절성·추천 질문 노드의 4-way 병렬 실행과 합류 구조 구현
- 60문항 평가기 구축 및 생성 모델·Judge 모델 분리
- 36문항 파일럿에서 기준을 충족하지 못한 전략 신뢰성 보강안을 확인하고 전면 롤백
...
- [최종 발표 자료](reports/final_presentation.html)
