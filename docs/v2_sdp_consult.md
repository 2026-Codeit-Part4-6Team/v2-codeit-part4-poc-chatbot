# 소프트웨어 개발 계획서 (SDP)
## 컨설턴트 챗봇 모델 개발 담당 전용 — 매출부스터 통합 플랫폼
...
[역할 ①] 데이터 공급자
   ...
   → 즉 컨설턴트 챗봇 모델 개발 담당자가 늦으면 기본 챗봇 담당자 작업이 지연된다.
...
| **업종 벤치마크 KB 데이터 작성** | JSON 내용 채우기 (전민재 PM과 분담 — §2.5) |
| **골든 데이터셋 · 평가 스크립트** | 컨설팅 답변 품질 측정 코드 |
| **OpenAI 429 대응** | 모델 호출 측 요청 간격·재시도·토큰 절감 |
| **API 계약 준수** | 서비스 개발 담당자가 호출할 응답 JSON 형식 준수 |
...
| **업종 벤치마크 인덱싱·검색 코드**(FAISS) | **기본 챗봇 담당자** ※ 데이터 작성만 컨설턴트 챗봇 개발 담당자 영역 |
| **`build_basic.py` 그래프 조립** | 기본 챗봇 담당자 |
| **★ 지오코딩(NAVER Maps) 호출** | **서비스 개발 담당자** ← v1.1 명확화(§2.8) |
| **`users` 테이블 lat/lng 저장** | 서비스 개발 담당자 |
| **캐시 테이블 스키마 생성**(DDL) | **서비스 개발 담당자** ※ 데이터 적재·조회 로직은 컨설턴트 챗봇 모델 개발 담당자가 직접 구현 |
...
>    → 컨설턴트 챗봇 개발 담당자 본인 편의만 생각해 설계하면 안 되고, **D1에 조회 함수 시그니처를 합의**해야 한다(§2.4)
...
def get_market(lat: float, lng: float, radius: int = 500) -> dict:
    """상권 조회 — Cache-Aside(캐시 우선, 캐시 미스 발생 시 API 실시간 호출). 실패 시 None."""
    return {
...
| **캐시 테이블 DDL(스키마 생성)** | **서비스 개발 담당자** | DB 테이블, 스키마 설계 서비스 개발 담당 영역 — 컨설턴트 챗봇 모델 개발 담당자는 **데이터 적재·조회 로직만** 구현 |
...
| 일자 | 작업 내용 | 완료 기준(DoD) |
| --- | --- | --- |
| **D1** 8/10(월) | · **★ 기본 챗봇 모델 개발 담당자에게 `get_market()`·`get_trend()` mock 스텁 제공**(§2.4) ← **최우선**<br>· **서비스 개발 담당자와 API 계약(JSON 형식) 합의**<br>· **★ 서비스 개발 담당자에게 확인 2건**: ①요청에 `store.lat/lng` 포함 여부(§2.8) ②캐시 테이블 DDL 생성 요청<br>· **전민재 PM과 검증 함수 3종 인터페이스 합의 + pass 스텁 수령**<br>· **PM 주재 GraphState 공통 규격 확정**<br>· LangGraph 프로젝트 구조 + GraphState 정의<br>· 상권 API·트렌드 API 키 발급 확인 | 그래프가 빈 노드로 START→END 실행됨<br>**상대가 컨설턴트 챗봇 모델 개발 담당자가 구현한 함수를 import 가능** |
...
| DB | **SQLAlchemy + SQLite3** | **테이블 DDL은 서비스 개발 담당**, 데이터 적재·조회는 컨설턴트 챗봇 모델 개발 담당자 영역 |
...
- **파일 관리권**: `cache/`·`graph/build_consultant.py`·`nodes/*_for_consult.py` 는 **컨설턴트 챗봇 모델 개발 담당자가 관리**,
  `validation/` 은 **전민재 PM**, `build_basic.py`·`nodes/*_for_copy.py` 는 **기본 챗봇 담당** → 머지 충돌 방지
...
- 평가 스크립트는 **직렬 실행 + 요청 간 time.sleep**. 병렬 호출 금지
...
| # | 리스크 | 발생 확률 | 영향 | 대응책 |
| --- | --- | --- | --- | --- |
| **R1** | **★ 컨설턴트 챗봇 개발 담당자 데이터 계층 지연 → 기본 챗봇 담당자 대기** | 중 | **치명적** | **§3.2 데이터 먼저 전략** — 1주차에 완성. D1에 mock 스텁 선제공(§2.4) |
...
| R6 | **리뷰 크롤링 실패**(선택자 변경·봇 차단) | **높음** | 낮음 | **[COULD] 등급** — 실패 시 개발 대상 제외(§3.3). 리뷰 없이도 전략 나오게 설계 |
...
> **가장 큰 리스크는 R1이다.**
> 다른 담당자와 달리, **컨설턴트 챗봇 모델 개발 담당자가 늦으면 다른 작업자 업무가 지연된다.** 그래서 §3.2 "데이터 먼저" 전략이 이 SDP의 핵심이다.
...
- [ ] 서비스 개발 담당자 구현한 화면에서 컨설턴트 챗봇 결과가 표시된다
