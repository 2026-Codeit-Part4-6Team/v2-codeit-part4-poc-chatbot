...
**우리가 만드는 것**: 소상공인을 위한 **통합 플랫폼**. 챗봇이 2개다.
- **기본 챗봇** — 광고 문구 + 광고 이미지를 만들어 준다.
...
| 역할 | 먼저 볼 곳 |
| --- | --- |
| 처음 합류 | §1 문서 개요 → §2.1 두 챗봇 구조 → §7 파이프라인 그림 |
| 백엔드/API | §3 기능 요구사항 → §5 데이터 요구사항 → §6 인터페이스 |
| 모델 개발 | §4 비기능(성능) → §7 파이프라인 → §9 평가 지표 |
| 인프라/배포 | §4 비기능(배포·429 대응) → §8 디렉토리 구조 |
| 프론트엔드 | §2.2 사용자 흐름 → §6.1 UI 상세 |
...
## 🔧 v6 → v7 수정 이력 (총 9건) — 무엇이 왜 틀렸는가

> 아래는 **틀린 것을 고친 기록**이다. 나머지 내용은 v6 그대로다.

### [수정 1] 파이프라인 그림 — 이미지 생성 노드의 위치 (⚠️ 가장 중요)
- **무엇이 틀렸나**: v6 파이프라인 그림은 **광고 이미지 생성을 카피 생성과 "병렬"로** 그려,
  라우팅에서 곧장 갈라져 나오게 표현했다.
- **왜 틀렸나**: 같은 문서 §3의 **FR-06이 이미지 생성의 입력을 "문구+스타일"로 정의**하고 있다.
  즉 이미지는 **카피가 먼저 나와야** 만들 수 있다. 병렬로 그리면 "아직 없는 문구"를 입력으로 쓰는 셈이라
  그림과 기능 정의가 서로 모순된다. (실제 POC 코드도 `best_copy`를 받아 이미지를 그렸다.)
- **어떻게 고쳤나**: `규제 준수 검증`을 통과한 **확정 카피 → 이미지 생성 → 이미지 자동 검수` 순서로 직렬 배치.
  FR-06의 입력도 "**확정 카피(FR-21 통과) + 스타일**"로 명확히 했다.

### [수정 2] 파이프라인 그림 — 컨설턴트 챗봇 경로 누락
- **무엇이 틀렸나**: v6는 라우팅 노드에 "기본 챗봇 / 컨설턴트 챗봇 분기"라고 써놓고,
  정작 **컨설턴트 챗봇이 지나가는 길은 그리지 않았다.**
- **왜 틀렸나**: §8 디렉토리 구조에는 `build_consultant.py`, `strategy.py`가 이미 있는데
  그림에 없으면, 컨설턴트 담당자는 **자기 기능이 어디로 흐르는지 알 수 없다.**
- **어떻게 고쳤나**: 그림에 **[레인 B] 컨설턴트 챗봇**을 신설 —
  `라우팅 → (상권·벤치마크 컨텍스트) → 마케팅 전략 제안 → 답변 생성`,
  그리고 EX-05 연계선(전략 승인 시 레인 A 트리거)을 점선으로 추가했다.

### [수정 3] 검색어 트렌드 API 경로를 "확정된 것처럼" 적음
- **무엇이 틀렸나**: v6는 `https://naverapihub.apigw.ntruss.com/search-trend/v1/search`를
  **확정 엔드포인트처럼** 적었다.
- **왜 틀렸나**: 이 경로는 **공식 명세로 확인되지 않은 추정값**이다.
  실제 NAVER API HUB의 검색 API는 `/search/v1/news`처럼 **`/search/v1/{종류}` 패턴**을 쓴다.
  잘못된 URL을 명세서에 박아두면 담당자가 그대로 코딩했다가 404로 시간을 버린다.
- **어떻게 고쳤나**: 베이스 URL만 확정 표기하고, **"경로는 공식 명세서에서 확인 후 확정"**으로 바꿨다.

### [수정 4] 트렌드 API 호출 한도가 아예 빠져 있었음 (429 설계와 직결)
- **무엇이 빠졌나**: v6 §4.1은 429 대응을 길게 설명하면서도,
  **정작 우리가 쓸 트렌드 API의 실제 한도 숫자가 하나도 없었다.**
- **왜 문제인가**: 한도를 모르면 "요청 간격을 얼마로 둘지"를 정할 수 없어 설계가 공허해진다.
- **어떻게 고쳤나**: 확인된 수치를 §6에 명시 —
  **Data Lab(검색어 트렌드) 월 최대 50,000건 · API Key당 최대 50 RPS · 초과 시 429 반환**,
  **한 요청당 최대 5개 그룹 × 그룹당 5개 키워드**.

### [수정 5] NCP Maps 무료 이용량 "월 300만"을 단정함
- **무엇이 틀렸나**: v6는 지오코딩 무료량을 **"무료 월 300만"으로 확정 서술**했다.
- **왜 틀렸나**: Maps API 무료 이용량은 **대표계정 1개에 한해 API별로** 제공되며,
  구간이 넓고 **정책이 변경된 이력**이 있다(구 AI NAVER API 경로의 지도 API 무료량 종료 안내 등).
  숫자를 단정하면 **"무료인 줄 알고 썼다가 과금"**되는 사고로 이어진다.
- **어떻게 고쳤나**: 숫자 단정을 제거하고 **"콘솔에서 현재 무료량·요금을 직접 확인하고 사용량 상한을 설정"**으로 변경.

### [수정 6] 이미지 평가 기능에 ID가 2개 (FR-29 / FR-13b)
- **무엇이 틀렸나**: §3에는 `FR-29 이미지 품질 평가`, §9.3에는 `FR-13b 이미지 품질 평가`가 있었다. **같은 기능에 ID 2개.**
- **왜 틀렸나**: 요구사항 ID는 **추적(traceability)의 열쇠**다. ID가 갈리면
  "FR-13b는 누가 하죠? FR-29랑 다른 건가요?" 같은 혼선이 생기고, 진척 관리가 깨진다.
- **어떻게 고쳤나**: **FR-29를 정식 ID로 삼고 FR-13b는 별칭**으로 표기(`FR-29 (=FR-13b)`).

### [수정 7] EX-01과 EX-02가 사실상 같은 내용
- **무엇이 틀렸나**: EX-01 "경쟁사 리뷰 감성·차별점 분석"과 EX-02 "경쟁사 리뷰 정성 분석"이
  둘 다 "사전 배치 캐시 기반 리뷰 분석"으로 **중복 서술**돼 있었다.
- **왜 틀렸나**: 담당자가 둘을 각각 구현하려 들면 **같은 일을 두 번** 한다.
- **어떻게 고쳤나**: **역할을 앞뒤로 분리** —
  **EX-02 = 리뷰 수집(배치, review_cache 적재)**, **EX-01 = 수집된 캐시 분석(감성→차별점)**.
  즉 EX-02가 선행, EX-01이 후행이다.

### [수정 8] `generations.plan` / `image_path`가 문서 안에서 충돌
- **무엇이 틀렸나**:
  (a) §2.1은 "**free/paid로 나누지 않는다**"고 선언했는데 §5 테이블엔 `plan | free/paid`가 그대로 있었다.
  (b) §4는 "이미지는 **base64 인라인**, 파일경로 아님"이라 했는데 `image_path`(파일경로) 컬럼이 함께 있었다.
- **왜 틀렸나**: 스키마와 정책이 어긋나면 **개발자마다 다르게 구현**한다.
  누구는 plan으로 기능을 막고, 누구는 image_path에 저장한다.
- **어떻게 고쳤나**: 컬럼은 **삭제하지 않고 의미를 재정의** —
  `plan` = **요금 등급 표기용**(기능 제한 용도 아님, 실제 제어는 credits·api_token_quota),
  `image_path` = **레거시**(미사용, 실제 렌더링은 image_b64). ERD 그림에도 같은 주석을 표기했다.

### [수정 9] FID를 "텍스트-이미지 정합성" 축에 넣음
- **무엇이 틀렸나**: §9.3 축 1(정합성)에 **FID가 포함**돼 있었다.
- **왜 틀렸나**: **FID는 정합성 지표가 아니다.** FID는 생성 이미지 *집합*과 실제 이미지 *집합*의
  **분포 거리**를 재는 값으로, **프롬프트와 이미지가 맞는지는 전혀 보지 않는다.**
  "여름 카페 문구"를 넣었는데 겨울 산 사진이 나와도, 사진이 자연스럽기만 하면 FID는 좋게 나온다.
- **어떻게 고쳤나**: FID를 **축 2(미적 품질·분포)의 참고 지표**로 이동하고,
  "샘플 수백 장 확보 시에만 유의미"라는 조건을 명시했다.
...
│   └── reports/                    # 평가 결과 리포트
├── docs/
│   ├── v7_sw_req_spec.md           # 본 문서(README)
│   ├── erd_v7.png                  # ★ERD 다이어그램
│   ├── pipeline_v7.png             # ★파이프라인 다이어그램
│   └── 보고서.pdf                   # 최종 제출 보고서
├── scripts/
