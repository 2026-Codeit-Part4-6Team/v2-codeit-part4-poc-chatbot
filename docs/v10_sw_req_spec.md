...
**핵심 흐름(v10에서 완성됨)**: 
1) 로그인 → 사이드바에서 챗봇 선택 → 대화 → 결과물. 하루 무료 횟수가 있고 초과 시 크레딧 차감.

2) 로그인 → 챗봇 선택 → 대화 → **생성 전 크레딧 동의** → **대기 화면** → **제안 3안 확인 후 승인·발행**.
...
> add3와 수정 스토리보드를 반영했다. **핵심은 "v9까지 시안이 없어 미정이던 화면들이 스토리 보드 시안이 작성된 것"이다.**
> v9의 나머지 내용은 그대로다.
...
| 서비스명 통일 | 문서=AdCopilot | ⚠️ **서비스명 통일 권장**(계속) |
...
## 📌 (v10) 팀에서 확정할 것 — 통합 체크리스트 (갱신)

### 🔴 개발 차단 이슈 (담당자 지정 후 논의)
- [ ] **일일 무료 한도** 5회 vs 10회 (스토리보드 수치 모순 #1)
- [ ] **회당 크레딧 차감액** 10 vs 50, 문구/이미지 차등 여부 — 단, S-14 동의창은 **"10 크레딧"**, 마이페이지 이력은 **"-50"** 으로 **여전히 불일치**
...
- [ ] **위치 입력 범위** — 가입만(CASE 1) vs 가입+사이드바 수정(CASE 2).
- [ ] **데이터 보관기간** — 생성 이력·캐시별 기간 + 삭제 방식(하드/소프트)
