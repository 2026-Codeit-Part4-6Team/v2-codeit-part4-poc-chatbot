...
- **핵심 변경(v2 대비)**:
  ...
  (3) 이미지 생성 → **로컬 SDXL + Triton 확정**(GPU 활용, 평가 가점)
...
(d) 플랫폼별 광고 문구(무료)와 문구+이미지(유료)를 생성한다.
...
| ID | 기능 | 입력 | 출력 | 담당 |
| --- | --- | --- | --- | --- |
| FR-01 | 주소→좌표 지오코딩 | 주소 문자열 | lat, lng | 윤승준 |
| FR-02 | 상권 정보 조회 | 좌표+반경(≤2000m) | 반경 내 매장 리스트(업종3단계·좌표·행정동 포함) | 윤승준 |
| FR-03 | 상권 정량 분석 | FR-02 결과 | 경쟁밀도·동일업종수·최근접거리·업종다양성 | 윤승준 |
| FR-04 | 업종 트렌드 조회 | 업종·키워드 | 검색어 트렌드 추이 | 전민재 |
| FR-05 | 플랫폼별 문구 생성 | 소재+플랫폼(인스타/블로그/현수막) | 플랫폼별 문구 3안 | 전민재 |
| FR-06 | 광고 이미지 생성(유료) | 문구+스타일 | 광고 이미지(SDXL) | 김재헌 |
| FR-07 | 게시물 생성(유·무료) | 생성요청+플랜 | 무료:문구 / 유료:문구+이미지 | 김재헌 |
| FR-08 | 대안 제시·재생성 | 최초 생성물 | 1안/2안 선택지 | 조희원 |
| FR-09 | 꼬리질문 정보 보완 | 최초 질문 | 반문 질문→반영 결과 | 조희원 |
| FR-10 | 회원가입·로그인 | 이메일·비밀번호·가게정보 | 세션 발급 | 김재헌 |
| FR-11 | 크레딧 결제·차감 | 충전 요청 / 유료 생성 | 크레딧 잔액·차감 결과 | 김재헌 |
| FR-12 | 라우터 분기 | 사용자 질의 | 적합 노드로 라우팅 | 김재헌 |
| FR-13 | 품질 평가(개발용) | 생성물·참조데이터 | HitRate@k / MRR / LLM Judge | 김재헌 |

### [확장] 시간 허락 시 (필요 시 진행)
| ID | 기능 | 비고 |
| --- | --- | --- |
| EX-01 | 경쟁사 리뷰 정성 분석 | 네이버 플레이스 Selenium 크롤링. **약관·안정성 리스크로 사전 배치 수집 캐시만 사용**. 실시간 경로 금지 |
| EX-02 | 실시간 날씨·뉴스 반영 카피 | 기상청/뉴스 API. MVP는 트렌드만으로 시의성 확보 |
| EX-03 | 마케팅 전략 제안 | 상권 정량 분석→프로모션/타겟팅 전략 텍스트 |
| EX-04 | 컨설팅→콘텐츠 연계 | 전략 승인 시 문구 생성 자동 트리거 |
| EX-05 | 이미지 제품 보존 생성 | IP-Adapter/ControlNet으로 제품 사진 보존 광고 이미지(가이드 '이미지 만들기 3') |

## 4. 비기능 요구사항
- **성능**:
  - 텍스트 생성: OpenAI API(gpt 계열). 응답 목표 p95 < 15초.
  - 이미지 생성: **로컬 SDXL(Stable Diffusion XL) + Triton 서빙**(GPU L4 활용).
    - 구현 순서: ① diffusers로 SDXL 직접 로드·품질 확보 → ② ONNX/TensorRT 변환 →
      ③ Triton 서빙 + FastAPI↔Triton gRPC 연결. **①을 건너뛰고 Triton부터 붙이지 않는다.**
...
이번 v3의 핵심 메시지를 팀에 전달할 때 이렇게 프레이밍하세요:
...
**오늘 팀에서 확정할 것 3가지**: (1) NCP 계정 만들어서 트렌드 HUB 키 발급 담당자 지정(민재님), (2) 카카오 REST 키 발급(승준님), (3) `password_hash` 방식 최종 통일(SHA-256 유지 vs bcrypt 전환).

혹시 SDXL+Triton 구현 로드맵(diffusers 프로토타입 → ONNX 변환 → Triton 서빙)을 주차별 태스크로 쪼갠 표가 필요하시면, 재헌님께 넘길 수 있게 만들어드릴게요. 이게 지금 팀에서 기술 난이도가 가장 높은 부분이라서요.